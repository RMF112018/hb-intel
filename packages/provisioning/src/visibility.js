 ...
 */
export function getProvisioningVisibility(session, submittedBy) {
    if (!session)
        return 'none';
    const roles = session.resolvedRoles ?? [];
    // Admin audiences always see the full 7-step checklist.
    if (roles.some((r) => FULL_CHECKLIST_ROLES.includes(r)))
        return 'full';
    if (session.user.email === submittedBy)
        return 'full';
    // Leadership + Shared Services never receive provisioning notifications.
    if (roles.some((r) => NO_NOTIFICATION_ROLES.includes(r)))
        return 'none';
