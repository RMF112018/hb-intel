...
function AuditDetail({ event }: { event: IAcknowledgmentEvent }) {
  return (
    <div className="hbc-ack-audit-detail">
      ...
      {/* Timestamp or pending-sync indicator (D-02) */}
      {event.acknowledgedAt && !event.isPendingSync && (
        <time
          dateTime={event.acknowledgedAt}
          className="hbc-ack-audit-detail__time"
        >
...
function ExpertPartyRow({
  ...
  return (
    ...
    >
      <div className="hbc-ack-party-row__header">
      ...
      {/* Expert audit detail — shown for events that have occurred */}
      {event && status !== 'pending' && <AuditDetail event={event} />}
    </li>
  );
}
...
export interface ExpertAuditTrailProps {
...
export function ExpertAuditTrail({
  config,
  state,
  parties,
  currentUserId,
  ...
  onAcknowledge,
  onDecline,
}: ExpertAuditTrailProps) {
  return (
    <ul className="hbc-ack-party-list hbc-ack-party-list--expert" role="list">
      {parties.map((party) => {
