...
export class ProxyProjectRepository extends BaseRepository<IActiveProject> implements IProjectRepository {
  ...
  async getProjectById(id: string): Promise<IActiveProject | null> {
    ...
    try {
      const raw = await this.client.get<unknown>(buildResourcePath('projects', id));
  ...
  async createProject(data: Omit<IActiveProject, 'id'>): Promise<IActiveProject> {
    const raw = await this.client.post<unknown>(buildResourcePath('projects'), data);
    return parseItemEnvelope<IActiveProject>(raw);
  }

  async updateProject(id: string, data: Partial<IActiveProject>): Promise<IActiveProject> {
    this.validateId(id, 'Project');
    const raw = await this.client.put<unknown>(buildResourcePath('projects', id), data);
    return parseItemEnvelope<IActiveProject>(raw);
  }

  async deleteProject(id: string): Promise<void> {
    this.validateId(id, 'Project');
    await this.client.delete(buildResourcePath('projects', id));
  }

  async getPortfolioSummary(): Promise<IPortfolioSummary> {
    const raw = await this.client.get<unknown>(buildResourcePath('projects', 'summary'));
