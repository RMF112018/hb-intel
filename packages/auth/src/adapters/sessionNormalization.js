 ...
 */
export function createAuthFailure(code, message, recoverable, details, cause) {
 ...
 */
export function normalizeIdentityToSession(identity, restoreSource = 'provider') {
    // ALIGNMENT: Session structure per D-PH5C-03
    // Normalize identity into canonical session shape for shared auth/shell contracts.
    if (!identity.user?.id) {
        return {
            ok: false,
            error: createAuthFailure('access-validation-issue', 'Normalized session requires a user id.', false),
        };
    }
 ...
 */
export function restoreSessionWithinPolicy(session, policy) {
    ...
    if (!session) {
        return {
            ...
            failure: createAuthFailure('missing-context', 'No previous session available for restoration.', true),
        };
    }
    ...
    if (safeUntil && safeUntil.getTime() < now.getTime()) {
        return {
            outcome: 'reauth-required',
            shellStatusTransition: 'restore-reauth-required',
            failure: createAuthFailure('expired-session', 'Restore window is expired; reauthentication required.', true),
        };
    }
    ...
    if (ageMs > policy.safeWindowMs) {
        return {
            outcome: 'reauth-required',
            shellStatusTransition: 'restore-reauth-required',
            failure: createAuthFailure('expired-session', 'Session is outside safe restore policy window.', true, { ageMs, safeWindowMs: policy.safeWindowMs }),
        };
    }
    return {
        outcome: 'restored',
 ...
 */
export function ensureSupportedMode(mode, supported) {
    if (!supported.includes(mode)) {
        return {
            ok: false,
            error: createAuthFailure('unsupported-runtime', `Runtime mode '${mode}' is not supported by this adapter.`, false),
        };
    }
    return { ok: true, value: mode };
}
//# sourceMappingURL=sessionNormalization.js.map