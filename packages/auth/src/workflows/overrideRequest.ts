 ...
 */
export const DEFAULT_OVERRIDE_REQUEST_POLICY: AccessOverrideRequestPolicy = {
 ...
 */
export function validateStructuredOverrideRequest(
  command: StructuredAccessOverrideRequestCommand,
  policy: AccessOverrideRequestPolicy = DEFAULT_OVERRIDE_REQUEST_POLICY,
): AccessOverrideRequestValidationResult {
  const errors: string[] = [];
  ...
  if (command.businessReason.trim().length < policy.minimumBusinessReasonLength) {
    errors.push(
      `businessReason must be at least ${policy.minimumBusinessReasonLength} characters for governed approval review.`,
    );
  }
 ...
 */
export function createStructuredOverrideRequest(
  command: StructuredAccessOverrideRequestCommand,
  policy: AccessOverrideRequestPolicy = DEFAULT_OVERRIDE_REQUEST_POLICY,
): StructuredAccessOverrideRequest {
 ...
 */
export function toOverrideRequestInput(
...
function resolveRequestedExpiration(
  command: StructuredAccessOverrideRequestCommand,
