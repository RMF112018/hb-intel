...
import { DEFAULT_OVERRIDE_REQUEST_POLICY, createStructuredOverrideRequest, } from './overrideRequest.js';
 ...
 */
export const DEFAULT_EMERGENCY_ACCESS_POLICY = {
 ...
 */
export function evaluateEmergencyBoundary(command, policy = DEFAULT_EMERGENCY_ACCESS_POLICY) {
    ...
    const hasAuthorizedRole = command.requesterRoles.some((role) => policy.authorizedRoles.includes(role));
    ...
    if (command.normalWorkflowAvailable && !command.boundaryBypassReason) {
        return {
            allowed: false,
            reason: 'Emergency access rejected because normal workflow is available and no boundary bypass reason was supplied.',
        };
    }
    if (command.normalWorkflowAvailable &&
 ...
 */
export function runEmergencyAccessWorkflow(command, policy = DEFAULT_EMERGENCY_ACCESS_POLICY) {
    ...
    return {
        ...
        request,
        override,
        audit: mergeAudits(approval.audit, createAccessControlAuditEvent({
...
function mergeAudits(primary, secondary) {
    if (!primary) {
        return [secondary];
    }
 ...
 */
function logEmergencyAudit(command, overrideId) {
...
//# sourceMappingURL=emergencyAccess.js.map