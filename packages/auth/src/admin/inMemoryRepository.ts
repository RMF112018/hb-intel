 ...
 */
export function createInMemoryAccessControlAdminRepository(
  ...
  return {
    async getSnapshot(query?: AccessControlAdminQuery): Promise<AccessControlAdminSnapshot> {
    ...
    async reviewOverride(command: ReviewOverrideCommand) {
    ...
    async renewOverride(command: RenewOverrideCommand) {
      const index = findOverrideIndex(state.overrides, command.overrideId);
      if (index < 0) {
        return {
          ok: false,
    ...
    async resolveRoleChangeReview(command: ResolveRoleChangeReviewCommand) {
      const index = findOverrideIndex(state.overrides, command.overrideId);
      if (index < 0) {
        return {
          ok: false,
    ...
    async reviewEmergencyAccess(command: ReviewEmergencyAccessCommand) {
      const index = findOverrideIndex(state.overrides, command.overrideId);
      if (index < 0) {
        return {
          ok: false,
...
function commitWorkflowResult(
  state: InMemoryState,
  index: number,
  result: Awaited<ReturnType<AccessControlAdminRepository['reviewOverride']>>,
) {
...
function findOverrideIndex(overrides: AccessControlOverrideRecord[], overrideId: string): number {
  return overrides.findIndex((override) => override.id === overrideId);
}

function buildDefaultUsers(): AccessControlUserLookupRecord[] {
  return [
    {
...
function buildDefaultRoles(): BaseRoleDefinition[] {
  return [
    createBaseRoleDefinition({
      id: 'administrator',
      name: 'Administrator',
...
function buildDefaultOverrides(): AccessControlOverrideRecord[] {
