 ...
 */
export function AdminAccessControlPage({ repository = defaultAccessControlAdminRepository, initialSection = 'user-lookup', title = 'Access Control Administration', }) {
    ...
    const onReview = async (decision, override, reason) => {
    ...
    const onResolveRoleReview = async (override) => {
    ...
    const onReviewEmergency = async (decision, override) => {
    ...
    return (<section aria-label="Access control administration" style={{ display: 'grid', gap: 16 }}>
      <header>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <input id="hbc-admin-search" aria-label="Search admin access data" value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} placeholder="Search users, roles, overrides" style={{ minWidth: 280, padding: '8px 10px', borderRadius: 4, border: '1px solid #a0a0a0' }}/>
      ...
      <nav aria-label="Admin queue sections" style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        {SECTION_DESCRIPTORS.map((section) => (<button key={section.id} type="button" onClick={() => setActiveSection(section.id)} aria-pressed={section.id === activeSection} title={section.description} style={{
                borderRadius: 4,
                padding: '8px 10px',
...
function MetricChip({ label, value }) {
    return (<div style={{ border: '1px solid #d4d4d4', borderRadius: 6, padding: '8px 12px' }}>
...
function renderSection(params) {
    ...
    switch (params.activeSection) {
        ...
        case 'override-review':
            return (<OverrideQueueTable rows={params.snapshot.overrideReviewQueue} loading={params.loading} onApprove={(row) => params.onReview('approve', row)} onReject={(row) => params.onReview('reject', row, 'Rejected during admin queue review.')}/>);
        case 'renewal-queue':
        ...
        case 'role-change-review':
            return (<RoleReviewQueueTable rows={params.snapshot.roleChangeReviewQueue} loading={params.loading} onResolve={params.onResolveRoleReview}/>);
        case 'emergency-review':
            return (<EmergencyQueueTable rows={params.snapshot.emergencyReviewQueue} loading={params.loading} onApprove={(row) => params.onReviewEmergency('approve', row)} onReject={(row) => params.onReviewEmergency('reject', row)}/>);
        case 'audit-log':
...
function UserLookupTable({ rows, loading }) {
    if (loading) {
        return <p>Loading user lookup...</p>;
    }
    return (<table aria-label="User lookup table" style={tableStyle}>
...
function OverrideQueueTable(params) {
    ...
    return (<table aria-label="Override review queue" style={tableStyle}>
      <thead><tr><th>ID</th><th>Target User</th><th>Base Role</th><th>Change</th><th>Actions</th></tr></thead>
      <tbody>
        {params.rows.map((row) => (<tr key={row.id}>
            <td>{row.id}</td>
...
function RenewalQueueTable(params) {
    ...
    return (<table aria-label="Renewal queue" style={tableStyle}>
...
function RoleReviewQueueTable(params) {
    ...
    return (<table aria-label="Role change review queue" style={tableStyle}>
...
function EmergencyQueueTable(params) {
    ...
    return (<table aria-label="Emergency review queue" style={tableStyle}>
...
function AuditLogTable({ rows, loading }) {
    if (loading) {
        return <p>Loading audit log...</p>;
    }
    return (<table aria-label="Access control audit log" style={tableStyle}>
...
//# sourceMappingURL=AdminAccessControlPage.jsx.map