...
import {
  ...
  type ProjectHealthPulseSnapshot,
} from '../../index.js';
import { createMockProjectHealthPulseSnapshot } from '../../../testing/index.js';

describe('SF21-T01 scaffold boundaries', () => {
  ...
  it('exposes deterministic testing fixtures through the public testing entrypoint', () => {
    const snapshot: ProjectHealthPulseSnapshot = createMockProjectHealthPulseSnapshot();

    expect(snapshot.projectId).toBe('project-sf21-fixture');
    expect(snapshot.dimensions).toHaveLength(4);
    expect(snapshot.reasonCodes[0]?.code).toBe('scaffold-fixture');
  });
});
