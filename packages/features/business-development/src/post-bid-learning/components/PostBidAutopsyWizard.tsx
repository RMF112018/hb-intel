...
import { useBusinessDevelopmentPostBidLearning } from '../hooks/index.js';
import {
...
const STEP_LABELS: Record<WizardStepId, string> = {
  'outcome-context': 'Outcome + pursuit context',
  'evidence-references': 'Evidence and linked references',
  'findings-recommendations': 'Findings, root cause, and recommendation',
  'disagreements-approval': 'Disagreements, approvals, and readiness',
  'impact-preview-submit': 'Downstream impact preview and submit',
};
...
const createStepValidationMessage = (
  ...
  switch (stepId) {
    case 'outcome-context':
      return draftValues[stepId]?.trim() ? null : 'Document the outcome context before continuing.';
    case 'evidence-references':
      return draftValues[stepId]?.trim() ? null : 'Add evidence references before continuing.';
    case 'findings-recommendations':
      return draftValues[stepId]?.trim() ? null : 'Capture findings and recommendations before continuing.';
...
const BD_AUTOPSY_UNAVAILABLE_CONFIG: ISmartEmptyStateConfig = {
  resolve: (context) => ({
...
export const PostBidAutopsyWizard: React.FC<PostBidAutopsyWizardProps> = ({
  ...
  const actor = useMemo(() => createDefaultEditingActor('Business Development'), []);
  ...
  const adapter = useBusinessDevelopmentPostBidLearning({
    pursuitId,
  ...
  const wizardConfig = useMemo<IStepWizardConfig<typeof wizardItem>>(
    () => ({
      ...
      allowReopen: true,
      draftKey: (item) => `bd-post-bid-learning:${item.pursuitId}`,
  ...
  if (loading && !primitiveState.state.autopsy) {
    return (
      <HbcCard header={<HbcTypography intent="heading2">Post-bid autopsy</HbcTypography>}>
        <HbcSpinner label="Loading post-bid autopsy wizard..." />
      </HbcCard>
    );
  }
  ...
  if (!primitiveState.state.autopsy) {
    return (
      <HbcSmartEmptyState
        config={BD_AUTOPSY_UNAVAILABLE_CONFIG}
        context={{
  ...
  const renderStepContent = (): React.ReactElement => {
    switch (activeStepId) {
      case 'outcome-context':
        return (
          <HbcFormSection
            title="Outcome and pursuit context"
            description="Resume the autopsy with the current pursuit context and confidence framing."
          >
              <div style={layoutStyles.chipRow}>
              ...
              <div style={layoutStyles.twoColumn}>
                ...
                <HbcTextField
                  label="Client"
                  value={pursuitSnapshot?.clientName ?? 'Client not provided'}
                  onChange={() => {}}
                  disabled
                />
              </div>
              <HbcTextArea
                ...
                placeholder="Summarize what changed, what was learned, and why this outcome matters."
              />
              <HbcTypography intent="bodySmall">
      ...
      case 'evidence-references':
        return (
          <HbcFormSection
            ...
            description="Evidence completeness remains primitive-derived and visible at all times."
          >
            <HbcTextArea
              ...
              placeholder="Document interviews, cost artifacts, field observations, and related references."
            />
            <div style={layoutStyles.twoColumn}>
              {sections.state.sections.map((section) => (
      ...
      case 'findings-recommendations':
        return (
          <HbcFormSection
            title="Findings, root cause, and recommendation"
            description="Keep explainability visible and sourced."
          >
            <HbcTextArea
              ...
              placeholder="Capture the primary factor, recommendation, and reusable finding."
            />
            {renderAiSuggestions()}
      ...
      case 'disagreements-approval':
        return (
          <HbcFormSection
            ...
            description="Publication blockers and escalation state stay primitive-derived."
          >
            <div style={layoutStyles.chipRow}>
              <HbcStatusBadge
              ...
              <HbcStatusBadge
                ...
                label={primitiveState.publicationBlockers.publishable ? 'Publishable' : 'Publish blocked'}
              />
            </div>
            <HbcCheckbox
              label="I explicitly approve publication persistence once blockers are cleared."
      ...
      default:
        return (
          <HbcFormSection
            title="Downstream impact preview and submit"
            description="Benchmark and intelligence impacts are previewed before submission."
          >
  ...
  const coreWizard = (
    <div style={layoutStyles.body} data-testid="bd-post-bid-autopsy-wizard">
      <HbcCard
        header={
          <div style={layoutStyles.chipRow}>
      ...
      >
        <div style={layoutStyles.chipRow}>
          ...
          <HbcStatusBadge variant={toLifecycleVariant(autopsy.status)} label={formatStatusLabel(autopsy.status)} />
          <HbcTypography intent="bodySmall">
            Trust indicator: {adapter.state.trustIndicator.confidenceTier ?? 'Unavailable'} / {adapter.state.trustIndicator.reasonCount} reasons
          </HbcTypography>
        </div>
        ...
        <div style={complexityTier === 'Essential' ? layoutStyles.body : layoutStyles.wizard}>
          ...
          <div style={layoutStyles.body}>
            ...
            <HbcCard header={<HbcTypography intent="heading3">My Work placement</HbcTypography>}>
              ...
              <HbcTypography intent="bodySmall">
                Route label: {adapter.state.route ?? 'No route available'}
              </HbcTypography>
            </HbcCard>
          </div>
        </div>
    ...
    <>
      <div style={layoutStyles.shell}>
        {complexityTier === 'Essential' && !isEssentialExpanded ? (
          ...
          >
            ...
            <HbcTypography intent="bodySmall">
              Essential mode keeps the autopsy collapsed until the author resumes the guided flow.
            </HbcTypography>
          </HbcCard>
        ) : (
          coreWizard
      ...
      {complexityTier === 'Expert' && impactPreview ? (
        <HbcTearsheet
          ...
          title="Downstream impact preview"
          steps={[
            {
              id: 'impact',
              label: 'Impact',
      ...
      {!impactOpen && (
        <div>
          <HbcButton variant="ghost" onClick={() => setImpactOpen(true)}>
            Open downstream impact preview
          </HbcButton>
        </div>
      )}
