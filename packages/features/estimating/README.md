...
The `post-bid-learning` surface is a scaffold-only Estimating adapter over `@hbc/post-bid-autopsy`. It adds Estimating profile defaults, deterministic view projections, and telemetry mapping without duplicating primitive-owned evidence, confidence, taxonomy, governance, publication, or lifecycle logic.
