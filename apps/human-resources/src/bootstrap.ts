/**
 * Synchronous mock environment bootstrap for Human Resources webpart.
...
export function bootstrapMockEnvironment(): void {
  ...
  usePermissionStore.getState().setFeatureFlags({ 'org-chart': true, 'onboarding-tracker': true });
  useProjectStore.getState().setAvailableProjects(MOCK_PROJECTS);
  useProjectStore.getState().setActiveProject(MOCK_PROJECTS[0]);
  useNavStore.getState().setActiveWorkspace('human-resources');

  console.log(
    `[HB-BOOTSTRAP] Human Resources mock environment bootstrapped as: ${persona.name} (${persona.id})`,
  );
}
