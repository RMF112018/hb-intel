/**
 * Synchronous mock environment bootstrap for Operational Excellence webpart.
...
export function bootstrapMockEnvironment(): void {
  ...
  usePermissionStore.getState().setFeatureFlags({ 'process-metrics': true });
  useProjectStore.getState().setAvailableProjects(MOCK_PROJECTS);
  useProjectStore.getState().setActiveProject(MOCK_PROJECTS[0]);
  useNavStore.getState().setActiveWorkspace('operational-excellence');

  console.log(
    `[HB-BOOTSTRAP] Operational Excellence mock environment bootstrapped as: ${persona.name} (${persona.id})`,
  );
}
