/**
 * Synchronous mock environment bootstrap for Safety webpart.
...
export function bootstrapMockEnvironment(): void {
  ...
  usePermissionStore.getState().setFeatureFlags({ 'incident-tracking': true, 'inspection-forms': true });
  useProjectStore.getState().setAvailableProjects(MOCK_PROJECTS);
  useProjectStore.getState().setActiveProject(MOCK_PROJECTS[0]);
  useNavStore.getState().setActiveWorkspace('safety');

  console.log(
    `[HB-BOOTSTRAP] Safety mock environment bootstrapped as: ${persona.name} (${persona.id})`,
  );
}
