 ...
 *
 * The current implementation supports `projectSpotlight` only.
 * Additional destinations (e.g. `companyPulse`) register a canonical
 * URL here when they are wired; the resolver returns `undefined` for
 * anything not yet wired, which surfaces as a validation error
 ...
 *
 * Returns `undefined` for destinations the current implementation
 * does not wire. Callers must treat `undefined` as "no binding site
 * is authorized — fail closed".
 */
...
/**
 * Destinations the current authoring + publish pipeline actually
 * supports end to end. Derived from `DESTINATION_VALUES` by keeping
 * only the entries that register a canonical site URL (i.e. the
 * publish pipeline knows where to put the page).
 *
