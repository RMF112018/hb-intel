...
const entryFile = process.env.HB_PUBLISHER_ENTRY ?? 'src/mount.tsx';
...
export default defineConfig(({ command }) => ({
  ...
  resolve: {
    alias: {
      ...
      // App source files must NOT import from '@hbc/ui-kit' directly —
      // prefer the subpath entries above.
    ...
    ...(isProduction || command === 'build'
      ? {
          lib: {
            ...
            name: '__hbIntel_hbPublisher',
            formats: ['iife'],
            fileName: () => 'hb-publisher-app.js'
          }
        }
      : {}),
    rollupOptions: {
