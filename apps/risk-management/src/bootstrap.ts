/**
 * Synchronous mock environment bootstrap for Risk Management webpart.
...
export function bootstrapMockEnvironment(): void {
  ...
  usePermissionStore.getState().setFeatureFlags({ 'risk-matrix': true, 'contingency-tracking': true });
  useProjectStore.getState().setAvailableProjects(MOCK_PROJECTS);
  useProjectStore.getState().setActiveProject(MOCK_PROJECTS[0]);
  useNavStore.getState().setActiveWorkspace('risk-management');

  console.log(
    `[HB-BOOTSTRAP] Risk Management mock environment bootstrapped as: ${persona.name} (${persona.id})`,
  );
}
