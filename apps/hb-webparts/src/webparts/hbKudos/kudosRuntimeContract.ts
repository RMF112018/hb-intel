...
export const KUDOS_RUNTIME_OWNERSHIP: {
  ...
  public: {
    ...
    owns: [
      'hero + featured spotlight + recent stream section',
      'recent-row date spines + shared section-header grammar',
      'archive zone (section-header, pill toggle, full-width search, expand/collapse)',
      'terminal feed CTA (always-visible "Browse the full Kudos feed" product surface)',
      'article reader panel (viewer-role safe, KudosReaderShell)',
  ...
  companion: {
    ...
    owns: [
      'queue filter model + reducer (tabs, search, ownership, flags) — runtime/companionFilter.ts',
      'per-tab scope counts derived alongside the queue — runtime/useCompanionQueue.ts',
      'sticky control zone + active-filter chip bar with per-chip dismiss + Clear-all',
      'workspace hierarchy layers (header band, sticky control zone, queue region, row anatomy)',
      'five-zone queue-row anatomy (state rail, content, recipients, submission, date spine)',
      'queue-row left-edge workflow-state rail (data-workflow-status / data-admin-flag / data-overdue attribute selectors)',
      'governance patch planning + dispatch — runtime/useCompanionActions.ts',
      ...
      'action-family composition on the detail panel (review decision / publication & prominence / admin-review flag / ownership / destructive takedown) — components/DetailPanel.tsx',
      'three-phase bulk approve state machine with progress bar, per-failure reporting, skipped tally, and Retry-failed — runtime/useBulkApproval.ts + components/BulkActionBar.tsx',
      'role-aware detail panel with audit timeline (KudosGovernanceDetailShell)',
      'overdue reminder derivation + announcements',
      'degraded-runtime rendering and retry (role-resolution-failed, host-unconfigured, access-restricted, loading, load-failure) — components/CompanionDegradedStates.tsx',
    ],
    doesNotOwn: [
