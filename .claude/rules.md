...
This file is the top-level rule index for Claude Code work in the `hb-intel` repository. It should stay concise and route the agent to the correct detailed rule file instead of duplicating every policy here.
...
`CLAUDE.md` is the root operating brief. This file is the detailed rule index and routing layer.
...
For the full specialist-agent index, routing table, and escalation examples, use:
...
| Rule | File | Use When |
|---|---|---|
| 01 — Authority and Reading Order | `.claude/rules/01-authority-and-reading-order.md` | Choosing the right source of truth, reading order, specialist agent, or verification source. |
...
| 04 — Documentation Standards | `.claude/rules/04-documentation-standards.md` | Creating/updating docs, deciding doc placement, preserving canonical docs, or avoiding scratch-plan pollution. |
...
| 06 — Planning and Proposals | `.claude/rules/06-planning-and-proposals.md` | Preparing plans, proposals, better-path recommendations, saved planning artifacts, or canonical plan updates. |
...
| PCC Phase 3 Active Rules | `.claude/rules/projects/pcc-phase-3.md` | Any task touching the Project Control Center, especially Phase 3 Wave 2 shell-frame/UI-UX work. |
...
### 3. Use Specialist Agents When They Reduce Risk
...
### 4. Specialist Escalation Priority
...
Use `.claude/agents/hb-ui-ux-conformance-reviewer.md` when ownership, consistency, accessibility, responsive behavior, SPFx fit, or visual direction is material.
...
Use `.claude/agents/hb-implementation-plan-reviewer.md` for pre-execution plan review or post-execution validation when the work is prompt-package-driven, phase/wave-driven, risky, cross-cutting, or sensitive.

Use `.claude/agents/hb-commit-diff-auditor.md` after execution when the key question is whether the actual diff matches the approved scope and whether the commit summary accurately describes the work.
...
Use `.claude/agents/hb-docs-curator.md` when documentation placement, drift, redundancy, or authority overlap is material.
