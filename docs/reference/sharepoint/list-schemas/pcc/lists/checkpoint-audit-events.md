...
## 1. Objective
- Planned PCC schema/reference file for `PCC Checkpoint Audit Events`.
...
| Checkpoint Audit Event ID | CheckpointAuditEventId | Text | Yes | No | No | Yes | Indexed for query/view performance |
...
| Timestamp | Timestamp | Text | Yes | No | No | No | MaxLength=255 |
...
| Details JSON | DetailsJson | Note | Yes | No | No | No | RichText=false; Lines=6 |
| Impacted Records JSON | ImpactedRecordsJson | Note | Yes | No | No | No | RichText=false; Lines=6 |
| Redacted Fields JSON | RedactedFieldsJson | Note | Yes | No | No | No | RichText=false; Lines=6 |
