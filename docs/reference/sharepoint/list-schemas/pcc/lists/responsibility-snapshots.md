# PCC Responsibility Snapshots

## 1. Objective
- Planned PCC schema/reference file for `PCC Responsibility Snapshots`.
...
| Snapshot ID | SnapshotId | Note | Yes | No | No | No | RichText=false; Lines=6 |
| Project ID | ProjectId | Text | Yes | No | No | Yes | Indexed for query/view performance |
| Generated At UTC | GeneratedAtUtc | DateTime | Yes | No | No | No |  |
| Read Only | ReadOnly | Boolean | Yes | No | No | No |  |
| Counts JSON | CountsJson | Note | Yes | No | No | No | RichText=false; Lines=6 |
| Health Score JSON | HealthScoreJson | Note | Yes | No | No | No | RichText=false; Lines=6 |
