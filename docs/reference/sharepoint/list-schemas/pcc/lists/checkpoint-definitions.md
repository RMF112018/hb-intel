...
## 1. Objective
- Planned PCC schema/reference file for `PCC Checkpoint Definitions`.
...
| Checkpoint Definition ID | CheckpointDefinitionId | Text | Yes | No | No | Yes | Indexed for query/view performance |
| Source Module | SourceModule | Text | Yes | No | No | No | MaxLength=255 |
| Kind | Kind | Choice | Yes | No | No | No |  |
| Validation Rule IDs JSON | ValidationRuleIdsJson | Note | Yes | No | No | No | RichText=false; Lines=6 |
| Owner Role | OwnerRole | Text | Yes | No | No | No | MaxLength=255 |
...
| Default Approval Policy ID | DefaultApprovalPolicyId | Text | Yes | No | No | Yes | Indexed for query/view performance |
