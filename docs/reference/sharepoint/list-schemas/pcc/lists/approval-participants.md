...
## 1. Objective
- Planned PCC schema/reference file for `PCC Approval Participants`.
...
| Principal Key | PrincipalKey | Text | Yes | No | No | Yes | Indexed for query/view performance |
| Role | Role | Text | Yes | No | No | No | MaxLength=255 |
| Delegated From Principal Key | DelegatedFromPrincipalKey | Text | Yes | No | No | Yes | Indexed for query/view performance |
| Delegation Allowed | DelegationAllowed | Boolean | Yes | No | No | No |  |
| Acknowledged At UTC | AcknowledgedAtUtc | DateTime | Yes | No | No | No |  |
