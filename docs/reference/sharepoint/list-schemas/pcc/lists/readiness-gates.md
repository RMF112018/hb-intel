...
## 1. Objective
- Planned PCC schema/reference file for `PCC Readiness Gates`.
...
| Gate ID | GateId | Text | Yes | No | No | Yes | Indexed for query/view performance |
...
| Gate Label | GateLabel | Text | Yes | No | No | No | MaxLength=255 |
| Gate Status | GateStatus | Choice | Yes | No | No | Yes | Indexed for query/view performance |
| Posture | Posture | Choice | Yes | No | No | No |  |
| Open Blocker Count | OpenBlockerCount | Number | Yes | No | No | No |  |
| Pending Evidence Count | PendingEvidenceCount | Number | No | No | No | No |  |
...
| Related Readiness Item IDs JSON | RelatedReadinessItemIdsJson | Note | Yes | No | No | Yes | RichText=false; Lines=6 |
