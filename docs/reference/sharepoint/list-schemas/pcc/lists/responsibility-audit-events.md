# PCC Responsibility Audit Events

## 1. Objective
- Planned PCC schema/reference file for `PCC Responsibility Audit Events`.
...
| Event ID | EventId | Text | Yes | No | No | Yes | Indexed for query/view performance |
...
| Actor JSON | ActorJson | Note | Yes | No | No | No | RichText=false; Lines=6 |
| Entity Ref | EntityRef | Text | Yes | No | No | No | MaxLength=255 |
