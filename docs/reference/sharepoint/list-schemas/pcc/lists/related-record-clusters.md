# PCC Related Record Clusters

## 1. Objective
- Planned PCC schema/reference file for `PCC Related Record Clusters`.
...
| Cluster ID | ClusterId | Text | Yes | No | No | Yes | Indexed for query/view performance |
| Project ID | ProjectId | Text | Yes | No | No | Yes | Indexed for query/view performance |
| Root Record ID | RootRecordId | Text | Yes | No | No | Yes | Indexed for query/view performance |
| Root Record Type | RootRecordType | Choice | Yes | No | No | No |  |
| Related Record IDs JSON | RelatedRecordIdsJson | Note | Yes | No | No | No | RichText=false; Lines=6 |
| Edge IDs JSON | EdgeIdsJson | Note | Yes | No | No | No | RichText=false; Lines=6 |
