...
> **Schema Readiness Verification** — run `pnpm tsx scripts/verify-my-project-role-fields.ts` (read-only, no `--apply`) to prove whether the 14 canonical columns (`Note`) and `procoreProject` (`Text`) exist with the expected types. See `docs/reference/spfx-surfaces/my-dashboard/my-projects-schema-readiness.md` for the interpretation and remediation flow.
