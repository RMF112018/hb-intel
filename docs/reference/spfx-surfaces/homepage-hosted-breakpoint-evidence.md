...
| Solution version                  | `1.1.79.0` promoted deployment target; prior accepted target was `1.1.78.0` | same                                                                                           |
| Feature id                        | `c4e91f7d-8a23-4b56-9d0e-1f3c5a7b2d84`                | same                                                                                           |
| Feature version                   | `1.1.79.0` promoted deployment target; prior accepted target was `1.1.78.0` | same                                                                                           |
...
| HbHomepageWebPart manifest version| `1.1.79.0` promoted deployment target; prior accepted target was `1.1.78.0` | same                                                                                           |
...
| Artifact                                              | SHA-256                                                              |
|-------------------------------------------------------|----------------------------------------------------------------------|
| `dist/sppkg/hb-intel-homepage.sppkg`                  | promoted `1.1.79.0` artifact: `5c7e3c98760fcea37308f6d7270e78d544704cd3e3ce8fc780f86347a36526ef` |
| `dist/sppkg/hb-intel-homepage-effectiveness-proof.json` | see current file on disk after rebuild |
| `dist/sppkg/hb-homepage-package-truth-proof.json`     | see current file on disk after rebuild |
| `dist/sppkg/hb-homepage-shim-proof.json`              | `804d333cc06060928acb469f77847181f3a2c33bce6bc3bbbb46afeb0f9b18e4`   |

**Effectiveness proof key assertions** (from the current `hb-intel-homepage-effectiveness-proof.json`):

- `versionAuthorityAligned` — **pass** for the current ignored local artifact at `1.1.79.0`; this is the promoted deployment target after the audit/proof commit. The prior accepted target was `1.1.78.0`.
...
- Packaged app bundle: `ClientSideAssets/hb-homepage-app-507b9572.js` and the package-truth proof reports `freshness.pass: true` for the audited `1.1.79.0` artifact.
- Packaged shim entry for the HbHomepageWebPart id: `shell-entry-e0a11c44-e6d7-45d1-9af5-09ba0b68f5cf-bd35b9e8.js`.

**Package authority test.** `apps/hb-webparts/src/webparts/hbHomepage/__tests__/hbHomepagePackageAuthority.test.ts` enforces that `solution.version === features[0].version === webpart manifest version` and that duplicate homepage manifest copies plus launcher runtime version constant remain in lockstep. The build-time orchestrator (`tools/build-spfx-package.ts`) refuses to emit `.sppkg` when the authoritative solution, feature, and source manifest versions diverge. The focused test gate passes for `1.1.79.0`, which is now accepted as the promoted deployment target after the audit/proof commit.
...
1. Deploy the accepted `dist/sppkg/hb-intel-homepage.sppkg` for `1.1.79.0` to the tenant app catalog only after the audit/proof commit. Confirm the catalog shows `1.1.79.0`; retain the historical note that `1.1.78.0` was the prior accepted cutover target.
...
7. Confirm the three Foleon communications lanes render in their former homepage locations: Project Spotlight, Company Pulse / Newsroom, and Leadership Message.
8. Confirm all three Foleon lanes show preview when no active record exists, switch to live content with valid active records/placements, and do not depend on `window.__hbIntel_foleon`.
9. Confirm no horizontal overflow at desktop, tablet, phone, short-height, and narrowest-stable breakpoints.
10. For §7, mark each row Pass only when packaging §3 is pass AND §6 runtime markers are all present AND §5 prose contains no regressions.
