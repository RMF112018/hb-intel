...
| Solution version                  | `1.1.78.0` target; current dirty local proof shows `1.1.79.0` | same                                                                                           |
| Feature id                        | `c4e91f7d-8a23-4b56-9d0e-1f3c5a7b2d84`                | same                                                                                           |
| Feature version                   | `1.1.78.0` target; current dirty local proof shows `1.1.79.0` | same                                                                                           |
...
| HbHomepageWebPart manifest version| `1.1.78.0` target; current dirty local proof shows `1.1.79.0` | same                                                                                           |
...
| Artifact                                              | SHA-256                                                              |
|-------------------------------------------------------|----------------------------------------------------------------------|
| `dist/sppkg/hb-intel-homepage.sppkg`                  | current dirty local artifact: `a9893a18367e78f767b8ba6219cc3caa95af4fa34af091e8d7f56ec8d78f5f18` |
...
- `versionAuthorityAligned` — **pass** for the current ignored local artifact at `1.1.79.0`, but this does not close the accepted `1.1.78.0` Foleon communications cutover target.
...
- Packaged app bundle: `ClientSideAssets/hb-homepage-app-c1b687e6.js` and the package-truth proof reports `freshness.pass: true` for the current dirty local artifact.
...
**Package authority test.** `apps/hb-webparts/src/webparts/hbHomepage/__tests__/hbHomepagePackageAuthority.test.ts` enforces that `solution.version === features[0].version === webpart manifest version` and that duplicate homepage manifest copies plus launcher runtime version constant remain in lockstep. The build-time orchestrator (`tools/build-spfx-package.ts`) refuses to emit `.sppkg` when these diverge. The focused test gate passes, but the current ignored artifact set reports `1.1.79.0`; do not use it as evidence for the accepted `1.1.78.0` deployment target.
...
1. Deploy the accepted `dist/sppkg/hb-intel-homepage.sppkg` for `1.1.78.0` to the tenant app catalog. Confirm the catalog shows `1.1.78.0`; if the local artifact/proof reports `1.1.79.0`, stop and regenerate from the accepted source/version state before deployment.
