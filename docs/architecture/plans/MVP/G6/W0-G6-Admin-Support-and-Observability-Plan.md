...
| LD-05 | Alert routing targets for Wave 0: Teams channel for `critical` and `high` severity; email to `<tech-team-email>` for `medium` and `low` digest alerts. This routing contract is implemented in `@hbc/features-admin`'s `notificationRouter.ts` and consumed by the dispatch adapter. |
