...
| `packages/data-access` | @hbc/data-access | v0.1.3 | usable-but-incomplete | Ports/adapters layer architecture complete. Real SharePoint/Graph adapters in progress; mock adapters in use. Critical Phase 1 blocker for production data integration. Production contract documented and approved (OD-015, 2026-03-16). |
...
| `packages/shell` | @hbc/shell | v0.0.2 | usable-but-incomplete | Global navigation and layout. Structure complete; responsive hardening and accessibility verification pending. |
...
| `packages/ui-kit` | @hbc/ui-kit | v2.2.23 | pilot-ready | Wave 1 production readiness scorecard passed (ADR-0116); component maturity matrix published; UI ownership and conformance governance active. Entry gate satisfied for Wave 0 pilot. |
...
| `packages/step-wizard` | @hbc/step-wizard | v0.1.2 | usable-but-incomplete | Multi-step guided workflow primitive. Contracts, navigation, and state complete; adapter-specific step validation logic pending. |
| `packages/versioned-record` | @hbc/versioned-record | v0.0.2 | usable-but-incomplete | Immutable versioned record management with diff engine, history hooks, and UI components. Full API surface documented (VersionApi, useVersionHistory, HbcVersionHistory, HbcVersionBadge). SPFx-compatible components defined; PWA-only components (HbcVersionDiff) gated (D-08). Production adapters (post-bid-autopsy, score-benchmark, ai-assist integration) pending. |
...
| `packages/features/leadership` | @hbc/features-leadership | v0.0.0 | scaffold-only | Leadership domain feature layer. v0.0.0 indicates no production implementation. Leadership is not a standalone target application (Target-Architecture-Blueprint §6.2); packages retained as implementation containers for role-aware visibility surfaces within Personal Work Hub and Project Hub (Dev-Roadmap reframing note 2026-03-15). |
...
| `apps/leadership` | @hbc/spfx-leadership | — | scaffold-only | SPFx Leadership webpart. Minimal beyond webpart scaffold. Not a standalone target application; retained as implementation container for role-aware surfaces in Personal Work Hub and Project Hub (Target-Architecture-Blueprint §6.2). Depends on @hbc/features-leadership (v0.0.0). |
...
|------|----------|---|---|---|
| `@hbc/data-access` | Core Platform (A) | v0.1.3 usable-but-incomplete | Phase 1 depends on real SharePoint/Graph adapter completion and production validation | Adapter stubs remain; no real SharePoint integration |
| `@hbc/versioned-record` | Shared-Feature Primitive (C) | v0.0.2 usable-but-incomplete | Blocks `post-bid-autopsy`, `score-benchmark`, `ai-assist` real usage | Production adapters for post-bid evidence snapshots and score records pending; PWA-only diff viewer (D-08 constraint) limits SPFx surfaces |
...
| `@hbc/ai-assist` | Shared-Feature Primitive (C) | v0.0.1 scaffold-only | Blocks production AI-assisted workflows | Deferred from Phase 1 per D-010; activation timeline pending OD-016. Mandatory Pre-Implementation Research Directive not yet complete; Azure AI Foundry endpoint integration and Smart Insert/Trust Meter UI not implemented |
...
**Count note:** The task brief mentioned 50 workspace members; `current-state-map.md` §6 lists 57; this matrix classifies 57. The matrix count (57) is the authoritative classification total for readiness purposes:
...
- `@hbc/versioned-record` v0.0.2: Pre-assessment listed as `scaffold-only`. Research revealed detailed README with full API surface (VersionApi, hooks, components), storage model (inline vs. file-library routing), SPFx constraints, and testing exports. Upgraded to `usable-but-incomplete` — core implementation exists, production adapters pending.
...
- `docs/architecture/blueprint/current-state-map.md` (Authoritative inventory, v1.0, 2026-03-15, sections 3 and 6)
