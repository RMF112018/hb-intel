...
| ARC-02 | `HbcCanvasEditor` + `useCanvasEditor` required for edit-mode in secondary zone | ✅ Wired — `HbcProjectCanvas` renders `HbcCanvasEditor` internally when `editable` is true; secondary zone has `editable` enabled; tertiary zone has `editable={false}` per ADR-0118 (remediation 2-F/2-G, 2026-03-22) | **Critical** — Resolved |
...
| ARC-06 | Two isolated `useCanvasEditor` instances (secondary + tertiary) required | ✅ Structurally satisfied — two `HbcProjectCanvas` instances with separate `projectId` values (`"my-work-hub"` / `"my-work-hub-tertiary"`); secondary has `editable`; tertiary has `editable={false}` per ADR-0118 (remediation 2-F, 2026-03-22) | **High** — Resolved |
