...
**Maturity model:** All status classifications below use the [FIN-PR1 Financial Production-Readiness Maturity Model](financial/FIN-PR1-Financial-Production-Readiness-Maturity-Model.md). "Complete" in the Status column means the criterion is satisfied at the indicated maturity stage — it does not imply the capability is operational or production-ready unless explicitly stated. See FIN-PR1 §4 for anti-overclaiming guidance.

| # | Criterion | Status | Maturity Stage | Evidence | Notes |
|---|---|---|---|---|---|
| 6.1.1 | Budget CSV import — atomic validation, identity resolution, `canonicalBudgetLineId` stability across re-imports | Complete at contract level | Stage 3 — Implementation Scaffold | README Stage 7.1 T02 (`@hbc/features-project-hub` v0.1.1 — budget record model, canonical identity, atomic import validation); `BudgetPage` UI renders with mock data | P3-E4-T02 §2-§3; P3-G1 §4.1. Advancing to Stage 4 requires `IFinancialRepository` and real data wiring. |
| 6.1.2 | Separated cost model — `jobToDateActualCost`, `committedCosts`, `forecastToComplete` are always distinct; `costExposureToDate` = actuals + committed | Complete at contract level | Stage 3 — Implementation Scaffold | README Stage 7.1 T02 (separated cost fields in budget record model, v0.1.1) | P3-E4-T02 §3.2; P3-E2 §3.2. Contract-level types and computors verified; operational proof requires real data. |
| 6.1.3 | Versioned forecast ledger — Working / ConfirmedInternal / PublishedMonthly / Superseded lifecycle; no unlock-in-place; derivation-based editing | Complete at contract level | Stage 3 — Implementation Scaffold | README Stage 7.1 T03 (versioned forecast ledger with 4-state lifecycle, derivation-based editing, `@hbc/features-project-hub` v0.1.2) | P3-E4-T03 §3; P3-E2 §3.3. Lifecycle logic tested; no runtime persistence. |
| 6.1.4 | Forecast checklist gate enforced — confirmation blocked when required items incomplete or `staleBudgetLineCount > 0` | Complete at contract level | Stage 3 — Implementation Scaffold | README Stage 7.1 T03 (confirmation blocked on incomplete checklist or stale budget lines, v0.1.2) | P3-E4-T03 §4.3. Gate logic tested in isolation; operational enforcement requires data layer. |
| 6.1.5 | Financial Forecast Summary editing — PM-editable fields on working version only; all derived fields recompute correctly | Complete at contract level | Stage 3 — Implementation Scaffold | T04 contracts complete: `IFinancialForecastSummary` (16 editable + 12 derived fields), `ForecastSummaryEditableField`/`ForecastSummaryDerivedField` unions, validation contract, computors (revised contract, profit, margin, contingency); `ForecastSummaryPage` UI with mock data; 12 T04 contract tests | P3-E4-T04 §5; P3-E2 §3.2. Advancing to R4 requires `IFinancialRepository`. |
| 6.1.6 | GC/GR working model — version-scoped; editable on working version only; aggregate feeds Forecast Summary | Complete at contract level | Stage 3 — Implementation Scaffold | T04 contracts complete: `IGCGRLine` (3 editable + 3 derived fields), `GCGRCategory`, `IGCGRSummaryRollup` (GC/GR subtotals), variance/rollup computors; `GCGRPage` UI with mock data; 12 T04 contract tests | P3-E4-T04 §6. Advancing to R4 requires `IFinancialRepository`. |
| 6.1.7 | Cash Flow model — 13 actuals (read-only) + 18 forecast months; A/R aging read-only; cumulative chart with deficit shading | Complete at contract level | Stage 3 — Implementation Scaffold | README Stage 7.1 T05 (`ICashFlowActualRecord` 21 fields, `ICashFlowForecastRecord` 14 fields, `ICashFlowSummary` 14 fields, A/R aging display model, `@hbc/features-project-hub` v0.1.4); `CashFlowPage` UI renders with mock data | P3-E4-T05 §7. Computors tested; UI uses mock data. |
| 6.1.8 | Buyout sub-domain — dollar-weighted completion metric; `ContractExecuted` gate enforced via issued Subcontract Execution Readiness decision / gate projection per P3-E13; savings recognition and three-destination disposition workflow | Complete at contract level | Stage 3 — Implementation Scaffold | README Stage 7.1 T06 (`IBuyoutLineItem` 19 fields, 7 procurement statuses, `ContractExecuted` gate via P3-E13, three-destination savings disposition, `@hbc/features-project-hub` v0.1.5); `BuyoutPage` UI renders with mock data | P3-E4-T06 §8; P3-E13-T07 §1. Business logic tested; UI uses mock data. |
| 6.1.9 | Report-candidate designation — at most one `isReportCandidate = true` per project; P3-F1 publication handoff handler implemented | Complete at contract level | Stage 3 — Implementation Scaffold | B-FIN-03 contract closure: `IPublicationEligibilityResult` (5 blocker codes), `IFinancialPublicationRecord` (immutable with supersession), `IPublicationHandoffResult` (P3-F1), `IFinancialExportRun` (6 export types), `IPublicationRecoveryPosture`; `promoteToPublished()` + `designateReportCandidate()` functions; 10 contract tests | P3-E4-T03 §3.6; P3-E4-T09 §16. Advancing to R4 requires `IFinancialPublicationRepository`. |
| 6.1.10 | PER annotation on confirmed versions — working version not visible to PER; `canonicalBudgetLineId`-anchored; carry-forward on derivation | Complete at contract level | Stage 3 — Implementation Scaffold | README Stage 7.1 T08 (`IFinancialAnnotationAnchor` — version-aware field/section/block anchors, `ICarriedForwardAnnotation` with value-change flag and PM disposition, `@hbc/features-project-hub` v0.1.7) | P3-E4-T08 §15; P3-E2 §3.5. Annotation contracts defined and tested; not yet wired to runtime annotation rendering. |
| 6.1.11 | All Activity Spine events, Health Spine metrics, and Work Queue items implemented per P3-E4-T08 §14 | Complete at contract level | Stage 3 — Implementation Scaffold | README Stage 7.1 T08 (10 activity spine event types, 10 health spine metric definitions, 8 work queue item types, v0.1.7) | P3-E4-T08 §14. Contracts and type definitions tested; spine adapters not yet data-connected. |
| 6.1.12 | Spreadsheet replacement notes verified | In Progress | Stage 3 — Implementation Scaffold | Financial Replacement Crosswalk package (FRC-00 through FRC-05) maps all 9 operating files to runtime equivalents. Pending: runtime UI surfaces with real data, parallel-run validation. | P3-E3 §2; [`financial/FRC-00`](financial/FRC-00-Financial-Replacement-Crosswalk.md). Crosswalk documentation is Stage 3; actual replacement readiness requires Stage 7+. |
