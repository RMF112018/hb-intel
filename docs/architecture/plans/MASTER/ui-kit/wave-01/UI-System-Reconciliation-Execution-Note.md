...
| **SafetyFieldExcellence** | `HbcOperationalSurface` | Cleanly migrated (confirmed W01r-P10) | ~132-line thin consumer that fully delegates the entire surface grammar (section shell, header, featured highlight, severity-aware signal stack) to `HbcOperationalSurface`. The three local lookup maps (`EVENT_VARIANT_MAP`, `EVENT_ICON_MAP`, `EVENT_SEVERITY_MAP`) translate the Safety-only `SafetyFieldEventType` enum into generic shared-surface severity / icon / badge primitives and correctly stay local as consumer business semantics. Consumer-level visual proof published as new Storybook stories (`SafetyFieldExcellenceConsumer`, `Sparse`, `SignalsOnly`, `Mobile`) under `packages/ui-kit/src/HbcOperationalSurface/HbcOperationalSurface.stories.tsx`. See `docs/architecture/reviews/safety-field-excellence-ui-kit-migration-completion.md`. |
...
- The remaining largest local consumer (ProjectPortfolioSpotlight) still imports governed presentation-lane brand tokens but builds its own surface grammar locally (W01r-P08 brand-token governance).
