...
| 6.1.5 | Financial Forecast Summary editing — PM-editable fields on working version only; all derived fields recompute correctly | In Progress | Stage 2 — Architecturally Defined | | P3-E4-T04 §5; P3-E2 §3.2. Blocked on T04 data model (`IFinancialForecastSummary`, `IGCGRLine`). `ForecastSummaryPage` UI exists but types incomplete. |
| 6.1.6 | GC/GR working model — version-scoped; editable on working version only; aggregate feeds Forecast Summary | In Progress | Stage 2 — Architecturally Defined | | P3-E4-T04 §6. Blocked on T04. No GC/GR UI component yet. |
