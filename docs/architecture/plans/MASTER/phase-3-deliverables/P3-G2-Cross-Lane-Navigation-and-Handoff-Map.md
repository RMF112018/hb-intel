...
**Repo-truth audit — 2026-03-21.** The SPFx `BackToProjectHub` pattern exists for SPFx-to-PWA return navigation. The PWA deep-link handler processes arrival via route parameters per P3-B1 §6.1. The project registry (P3-A1) stores `siteUrl` for PWA-to-SPFx navigation. Cross-lane navigation rules are locked in P3-G1 §7, P3-B1 §6/§8, and P3-C3 §8. This specification formalizes those rules into a complete map.
...
SPFx MUST route to PWA when (P3-G1 §5.2):
...
|---|---|---|
| SPFx → PWA | Include `projectId` in deep-link URL path or query parameter | PWA resolves via route params (P3-B1 §2) |
...
| Work Queue | Navigate to work queue page (in-app) | Open work queue panel; "View All" escalates to PWA |
| Activity | Navigate to activity timeline (in-app) | Show recent activity tile; "View All" escalates to PWA |
...
5. **Module-level escalation patterns — controlled evolution**
   Per-module escalation patterns (§8) are defined but depend on module implementation. Each module must implement the escalation triggers during Phase 3 execution. Classified as **controlled evolution**.
...
| Field | Value |
|---|---|
| **Pass condition** | Cross-lane navigation works correctly: SPFx-to-PWA escalation preserves context, PWA-to-SPFx opens correct project site, return navigation functions, no context loss during handoff |
| **Evidence required** | P3-G2 (this document), SPFx-to-PWA deep links functional, PWA-to-SPFx siteUrl navigation functional, `returnTo` parameter processed, project identity preserved across lane transitions, error handling for invalid projects/access |
