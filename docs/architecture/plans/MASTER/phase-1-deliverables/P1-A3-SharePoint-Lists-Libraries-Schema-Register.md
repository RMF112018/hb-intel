...
| 12 | Governing Schema | P1-A4 schedule_activity entity. **Compressed model:** 16 canonical entities → 3 physical containers. `baselineStart`/`baselineFinish` carry primary baseline (baseline_number=0) flattened from `schedule_baseline`. `sourceExtrasJson` carries relationships, UDF values, and other flattened entity data. See canonical-to-physical reconciliation note in Section 5. |
...
| 7 | Lookups | `reportId` → LessonsReports, `categoryKey` → hub-site LessonCategories dictionary, `phaseEncounteredKey` → hub-site LessonPhases dictionary |
| 8 | Indexes | `reportId`, `categoryKey`, `phaseEncounteredKey`, `applicabilityScore` |
| 9–11 | Standard | Major versions / Project team / Project site provisioning |
