...
Perform the full Phase C validation and closeout pass.

This prompt must verify that the utility/discovery premiumization is complete, coherent, documented, and safe for the current HB Central homepage/SPFx lane.
...
- any shared/homepage-safe primitives touched for this phase
...
Verify that the final code does **not** over-promote narrow one-off utility/discovery choreography into shared ui-kit space.
...
Do not leave the repo in a state where the new utility/discovery posture exists but the docs still describe the old weak interaction model.

### 5. Write the final completion note
Produce a polished Phase C completion note that includes:
...
- recommendation on whether Phase C is complete and what should happen next
...
- a high-quality Phase C completion note exists in the repo
...
- path to the final Phase C completion note
