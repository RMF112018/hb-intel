...
2. What exact backend event currently starts the provisioning saga?
3. Does the backend preserve a meaningful distinction between `ReadyToProvision` and `Provisioning`, or is one effectively transitional in practice?
4. What Accounting actions exist today that rely on backend support?
5. Where does the current backend contract leave the Accounting UI in a dead end or ambiguous state?
6. Which server-side validations are implemented today?
7. Which validations described in repo docs are missing, partial, or overstated in code?
8. What duplicate-run protections exist today?
9. What durable request/run/status correlation exists today?
10. What documentation is materially out of sync with repo truth?
...
- the current provisioning trigger point is explicitly identified
