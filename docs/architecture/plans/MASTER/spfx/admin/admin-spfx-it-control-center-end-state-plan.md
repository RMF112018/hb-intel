...
**Version:** 01.000.038  
**Revision note:** Adds **Phase 12 testing, docs, runbooks, and config guidance** (P12-10). Updates `@hbc/features-admin` README: replaces Wave 0 limitations section with post-Phase-12 status showing resolved items (durable persistence, ErrorLogPage active, 3rd monitor live, resolve action, delivery tracking) and still-open items (email relay, 3 deferred monitors, 3 deferred probes, approval stub, webhook retry). Updates `apps/admin` README: Errors lane from Deferred to Active, Deferred Pages section replaced with Phase 12 observability notes. Updates `current-state-map.md`: adds section 7.2 Phase 12 Completions (8 items), renumbers Active but Incomplete to 7.3, renumbers Wave 1 deferrals to 7.4. Creates operator runbook notes (what operators see, data sources, severity/status interpretation, actions, escalation guidance, later-phase items). Creates config and retention guide (7 storage tables, retention targets, 5 polling intervals, environment variables, deployment requirements, adapter mode behavior, self-monitoring KQL). Documentation-only — no code changes, no verification needed. Preserves all prior phase content.
