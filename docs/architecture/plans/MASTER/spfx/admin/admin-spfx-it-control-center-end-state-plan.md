...
**Version:** 01.000.037  
**Revision note:** Adds **Phase 12 notification dispatch and operator action workflows** (P12-09). Hardens `TeamsWebhookDispatchAdapter` with delivery status tracking (`INotificationDeliveryRecord` recording delivered/failed/suppressed/skipped status per dispatch), acknowledged-alert suppression (suppresses re-notification for acknowledged alerts unless severity escalated), and 5-minute cooldown duplicate suppression. Adds `resolve()` action to `AdminAlertsApi` (sets `resolvedAt`, auto-acknowledges if not already acknowledged) and `resolve` method to `useAdminAlerts` hook/`UseAdminAlertsResult` type. Exports `INotificationDeliveryRecord` from integrations barrel. Alert action workflow documentation covers state machine (active→acknowledged→resolved with direct-resolve path), operator actions, dispatch channels and routing rules, 3-tier deduplication (monitor, backend, notification), delivery tracking, and production-ready vs limited feature inventory. 19 new tests covering state transitions, suppression for acknowledged/cooldown/escalation, delivery tracking for all statuses, and deduplication behavior. Validation: features-admin check-types/lint/build clean, 219 tests passed (15 files). Preserves all prior phase content.
