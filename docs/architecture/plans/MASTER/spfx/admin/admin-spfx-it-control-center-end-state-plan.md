...
**Version:** 01.000.036  
**Revision note:** Adds **Phase 12 SPFx unified observability console and routes** (P12-08). Replaces the deferred `ErrorLogPage` stub with a real error log surface querying the backend observability error store via `useObservabilityErrors` hook. ErrorLogPage implements domain/severity/classification filtering using `HbcSelect`, error event cards with `HbcStatusBadge` severity badges and correlation metadata (domain, source, classification, timestamp, runId, actionKey), smart empty states for truly-empty and filter-empty conditions, and session/loading/error guards following established page patterns. Updates Errors lane from `scaffold` to `active` in lane registry, removing the SF17-T05 deferral. Introduces `useObservabilityErrors` hook in `@hbc/features-admin` with React Query integration (30s refetch), filter state management, and query param composition. Admin app version bumped to 00.000.122. Operator console UX map documents route changes, page/component ownership, workflows, empty/loading/error states, and remaining gaps. Validation: features-admin check-types/lint/build/test clean (200 tests, 14 files), admin app check-types clean, build clean, lint 0 new errors (61 pre-existing in EntraLanePage/SharePointControlPage). Preserves all prior phase content.
