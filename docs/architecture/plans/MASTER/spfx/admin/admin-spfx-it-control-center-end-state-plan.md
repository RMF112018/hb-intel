...
**Version:** 01.000.033  
**Revision note:** Adds **Phase 12 backend observability APIs and telemetry normalization** (P12-05). Implements 15 HTTP API endpoints in `observability-routes.ts` covering alerts (list, detail, acknowledge, resolve, summary, ingest), probes (latest, submit, history, health), errors (list, detail, ingest), dashboard summary, and correlated run timeline. Wires observability stores into admin-control-plane service factory. Introduces 3 backend services: `assembleDashboardSummary` (combines alert + probe + error summaries in parallel), `assembleRunTimeline` (joins audit events + alerts + errors by runId with chronological sort), and `createObservabilityBridge` (fire-and-forget normalization of saga failures into error records and monitor evaluations into alert records, with failure-class mapping from provisioning taxonomy). API map documentation created with endpoint inventory, request/response shapes, auth expectations, correlation semantics, and query capabilities. 13 new tests covering dashboard assembly, timeline merging, bridge normalization with failure-class mapping, and fire-and-forget safety. Validation: functions check-types clean, lint 0 errors (122 pre-existing warnings), build clean, 1855 tests passed (98 files). Preserves all prior phase content.
