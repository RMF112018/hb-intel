...
**Version:** 01.000.032  
**Revision note:** Adds **Phase 12 durable persistence adapters and storage plumbing** (P12-04). Implements 3 observability stores in `backend/functions/src/services/admin-control-plane/`: `DurableObservabilityAlertStore` (alerts with deduplication, acknowledge, resolve, summary), `DurableObservabilityProbeSnapshotStore` (append-only probe snapshots with overall-status computation and staleness detection), and `DurableObservabilityErrorStore` (append-only error events with multi-filter query). Each store has a mock counterpart for test/dev mode, serialization round-trip helpers following Phase 4 null conventions, and typed interfaces in `types.ts`. 3 new Azure Table Storage tables: `ObservabilityAlerts`, `ObservabilityProbeSnapshots`, `ObservabilityErrors`. Barrel exports and service interfaces updated. 31 new tests in `observability-stores.test.ts`. Persistence adapter map documentation created. Validation: functions check-types clean, lint 0 errors (119 pre-existing warnings), build clean, 1842 tests passed (97 files). Preserves all prior phase content.
