...
**Version:** 01.000.035  
**Revision note:** Adds **Phase 12 cross-domain instrumentation and correlation adoption** (P12-07). Introduces `emitRouteError` fire-and-forget observability emitter with heuristic error classification (structural/permissions/transient/admin-class/unclassified). Instruments 9 route catch blocks across 3 admin domains: ProvisioningRollout (cancelRun, retryRun), WhiteGloveDeployment (launch, cancel, retry package/device, resolveCheckpoint), EntraControl (testConnection, updateConnectionPolicy). Each emitted error record carries domain, source, operation, runId, and actionKey correlation. Adoption map documents all instrumented domains, correlation keys, classification heuristics, extension points for future domains, and routes that cannot yet be instrumented (no catch blocks). 9 new tests covering emitter classification for all 5 error patterns, non-Error handling, fire-and-forget safety on store failure, and domain-specific emission. Validation: functions check-types clean, lint 0 errors (124 pre-existing warnings), build clean, 1864 tests passed (99 files). Preserves all prior phase content.
