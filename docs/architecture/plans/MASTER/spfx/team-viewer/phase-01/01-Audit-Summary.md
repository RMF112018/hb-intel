...
- plus follow-up scope for article-bound source resolution and a disabled-by-default but real bio/resume slide-out
...
## Follow-up requirement interpretation

Two additional requirements are now locked into the package:

1. `teamViewer` should not be treated as an isolated people list. It must resolve article-linked team members from the article system based on **host site / destination context**.
2. Team-member cards/rows must support a **real** bio/resume slide-out pattern, but that experience must ship **disabled by default** until enabled intentionally.

This shifts the recommended architecture further toward:
- a clean article-binding seam,
- explicit team-member child-record normalization,
- and a profile-detail shell that is built now but feature-flagged off by default.
