...
## Additional architecture implications from follow-up requirements

### New requirement A — article-bound source resolution
`teamViewer` now needs an explicit source-binding model that can map host site / destination context to the correct article record, then resolve ordered team-member child rows for that article.

This means the package must treat the uploaded article architecture as a planning input:

- `HB Articles` — authoritative article record
- `HB Article Team Members` — child records for ordered people rows
- `HB Article Destination Pages` — destination/page binding state that may help resolve which article is active on a given host site/page

**Why it matters:** this adds a real parent/child binding seam and moves `teamViewer` away from a generic property-driven people list model.

**Correction direction:** the code agent must:
- audit whether article-bound resolution already exists anywhere in repo truth,
- decide whether the source key should be host site URL, page URL, article id, or a supplied property fallback,
- normalize team-member child rows into a stable `TeamViewerPerson` view model,
- and explicitly document any schema gaps that prevent bio/resume content from being fully supported.

### New requirement B — real bio/resume slide-out, disabled by default
`teamViewer` now needs a true detail drawer path for each team member.

The drawer should learn from Kudos’ right-side shell mechanics:
- focus trap
- close behavior
- motion discipline
- host-safe offset behavior where relevant

But it must not reuse recognition content grammar.

**Why it matters:** this elevates `teamViewer` from a display-only surface to a partial interaction product and introduces detail-state, accessibility, and feature-flag requirements.

**Correction direction:** the code agent must:
- implement the drawer for real,
- define the person-detail contract for bio/resume content,
- add a feature flag / config lock so the drawer is disabled by default,
- and still prove the enabled path in harness/validation so the feature is not a stub.

### New likely schema gap
The uploaded article architecture clearly establishes article master + team-member child rows, but it does not yet explicitly show bio/resume fields on `HB Article Team Members`.

**Why it matters:** the code agent must not assume those fields already exist.

**Correction direction:** gap-register and design one of the following:
- article-team-member fields expanded to carry bio/resume data,
- a linked person-profile source,
- or an explicit `profileContent` child/source contract if repo truth proves that is cleaner.
