# Prompt 02 — Wave 6 Team & Access View Model and Adapter

## Objective

Create the Team & Access app-local view model and adapter that converts existing shared fixtures/read models into UI-ready state without changing shared models or adding backend wiring.
...
- `apps/project-control-center/src/surfaces/teamAccess/teamAccessAdapter.test.ts`
- `apps/project-control-center/src/surfaces/teamAccess/shared.ts` only if needed to delegate to the new adapter
- existing Team & Access surface tests, only if they need import/path updates
...
1. Keep `packages/models/src/pcc/TeamAccess.ts` unchanged unless the local gate proves an unavoidable type defect.
2. Build an app-local view model that represents:
   - audience state;
   - visible lanes;
   - member counts;
   - request status summaries;
   - pending review queue count;
   - approved-pending-execution count;
   - manual IT / backend-gated execution posture;
   - audit trail display rows.
3. Derive from existing `ITeamAccessPreviewModel` and fixture lane records.
4. Preserve manager persona list from `TEAM_ACCESS_MANAGER_PERSONAS`.
5. Preserve branch variations:
   - has-project-access;
   - needs-project-access;
   - access-manager.
6. Add state labels for:
   - draft preview;
   - submitted preview;
   - pending review;
   ...
   - rejected;
   - completed manual.
7. Ensure no controls execute anything.
8. Keep all logic pure and unit-testable.
...
- Adapter tests must cover all audience branches.
- Tests must prove request statuses map to explicit labels.
- Tests must prove `approved-pending-execution` renders a “pending execution / no permission change executed” posture.
- Tests must prove manager personas come from shared model truth, not invented values.
- Tests must prove fixtures including `external` and `guest` are display records only and do not trigger access execution.
...
```text
feat(spfx-pcc): add team access view model adapter
```

## Expected commit description

```text
Adds a PCC Team & Access app-local view model and pure adapter over existing shared TeamAccess fixtures.

Preserves shared model vocabulary, fixture-default behavior, preview-only posture, no permission execution, no backend route/client changes, no package/lockfile/manifest/workflow/deployment changes.
