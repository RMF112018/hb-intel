...
Finalize workbook source mapping documentation and `default_buyout_log_seed_structure.json` from the repo-resident workbook extraction.
...
- Every workbook column header must be included in `recordFields[]`.
- Workbook rows must be classified accurately.
- Template rows are source taxonomy only unless activated.
- Summary/formula/header/blank rows must not be active records.
- Preserve exact source row references from the workbook extraction.

## Commit Summary

`docs(pcc): map wave 13 buyout workbook sources`
