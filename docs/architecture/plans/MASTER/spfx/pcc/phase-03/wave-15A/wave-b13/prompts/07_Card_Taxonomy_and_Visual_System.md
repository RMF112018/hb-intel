# Phase 08 Prompt 07 — Card Taxonomy and Visual System

## Objective

Introduce or refine a card taxonomy so PCC cards communicate purpose, severity, source, and role more clearly.
...
Enhance `PccDashboardCard` and related CSS so cards no longer feel visually flat or interchangeable. Add optional taxonomy/state markers only if needed and backward compatible.
...
apps/project-control-center/src/tests/*DashboardCard*
...
- Optional prop additions to card primitive.
- CSS module enhancements.
- Test updates for new markers.

## Prohibited Changes

- Do not make entire cards clickable by default.
- Do not remove existing card data markers.
- Do not change global footprint defaults unless necessary and tested.

## Required Steps

1. Inspect current card primitive and tests.
2. Add optional `cardKind` and/or `severity` props only if needed for visual/evidence differentiation.
3. Keep all new props optional and backward compatible.
4. Add stable data markers such as `data-pcc-card-kind` and `data-pcc-card-severity` if props are added.
5. Refine card chrome: header layout, action slot alignment, subtle depth, left-edge accents, hover/focus-within states.
6. Preserve span overrides and row-span measurement.
7. Add tests for new markers/default behavior.

## Acceptance Criteria

- Cards visually differentiate action, source, analytics, gateway, reference, and selected module patterns.
- No existing card breaks due to new props.
- Direct-child bento invariant preserved.
- Span override behavior preserved.
- No card becomes clickable unless accessible and intentional.
