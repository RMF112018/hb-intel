# Phase 08 Prompt 16 — Accessibility and Regression Tests

## Objective

Harden accessibility and regression coverage after visual and interaction enhancements.
...
Add or update tests to prove Phase 08 did not regress keyboard navigation, semantic roles, active panel ownership, bento invariants, disabled states, chart accessibility, no-writeback posture, or no duplicate header cards.
...
- Do not delete existing tests unless replacing with stronger coverage.
- Do not weaken exact-copy tests without a clear copy contract.
- Do not skip full test suite in closeout.

## Required Steps

1. Run the full PCC test suite first to establish current state.
2. Inspect existing tests for shell, tabs, Project Home, Documents, analytics, Cost & Time, Systems Administration, and duplicate header coverage.
3. Add tests for Phase 08 markers and behaviors introduced in earlier prompts.
4. Test keyboard navigation/focus for tabs and module menus.
5. Test disabled reason copy and no false affordance.
6. Test analytics accessible summaries and no chart-only meaning.
7. Test no `echarts-for-react` remains.
8. Run full test and typecheck.

## Acceptance Criteria

- Full PCC vitest passes.
- Typecheck passes.
- Accessibility-critical behaviors covered.
- No duplicate header card regression.
- No active-panel ownership regression.
- No bento direct-child regression.
- No Sage/no-writeback regression.
