# Repo-Truth Audit and Governing Document Check
...
Verify root Playwright status, PCC package status, existing live SharePoint pattern, governing document existence, PCC scorecard path, `data-pcc-*` selector availability, package/version files, and .gitignore artifact safety. No source edits unless required to record an audit note requested by the user.
