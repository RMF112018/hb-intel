...
Strengthen shell/canvas host-fit, visual separation, and layout resilience while preserving SharePoint-hosted constraints.
...
Improve the shell and canvas presentation so PCC feels more productized inside SharePoint while preserving no-sidebar, no-full-page-takeover, active-panel ownership, and bento direct-child rules.
...
- CSS refinements to shell/canvas and bento surface.
- Test updates for shell ownership and host-fit markers.
- Small non-breaking data marker additions if useful for evidence.

## Prohibited Changes

- Do not reorder shell children unless necessary and explicitly justified.
...
1. Inspect current shell/canvas CSS and tests.
2. Improve visual seam between SharePoint host, PCC command layer, and bento canvas.
3. Ensure `main[role="tabpanel"]` remains the only active-surface-panel owner.
4. Defend against horizontal overflow and clipping.
5. Keep `PccBentoGrid` direct-child behavior intact.
6. Add or update tests for active panel ownership and no card-level ownership if needed.
7. Run focused tests and typecheck.

## Acceptance Criteria

- No PCC sidebar introduced.
- No horizontal overflow introduced by shell/canvas changes.
- `data-pcc-active-surface-panel` remains on shell `main[role="tabpanel"]`.
- Bento direct-child invariant remains intact.
- Shell/canvas looks more intentional and productized.
