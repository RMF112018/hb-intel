# Normalize comment and policy language to repo truth
...
Issue reference:
- `P3-1` from the publisher backend audit findings register
...
Objective:
Bring comments and policy language back to exact repo truth.

Issue to close:
Several comments overstate guarantees around in-place republish identity and superseded prior bindings.

Required outcomes:
- Comments describe the actual implemented behavior, not intended future behavior.
- Readers are not misled into believing the seam is already stronger than it is.

Required work:
1. Scrub the named files for comments/messages that overstate guarantees.
2. Rewrite them to match the actual implemented behavior after earlier prompts in this package are applied.
3. If earlier prompts change behavior materially, make comments precise to that final state.
4. Avoid vague aspirational language; be exact.

Acceptance standard:
- A future developer can trust comments as repo truth.
- No policy/comment language claims behavior that code does not enforce.
