# Prompt-08 — Add the missing `HB Article Promotion Rules` seam and wire its effects
...
Objective:
Introduce a real promotion-rules repository seam and apply it where the tenant model expects it to influence authoring defaults and publish outcomes.
...
- new repository/mapper files
- `apps/hb-webparts/src/webparts/articlePublisher/ArticlePublisher.tsx`
- `publishOrchestrator.ts`
- tests

Required changes:
1. Add a list descriptor, contract, row mapper, repository, and tests for `HB Article Promotion Rules`.
2. Decide and implement where those rules should apply in the current sprint: authoring defaults, publish-time rollup flags, and manual override gating as appropriate.
3. Keep the implementation tightly bounded to tenant-backed rule effects; do not invent extra platform behavior.
4. Document the exact application points in code comments or a local closure note.
...
- A real promotion-rules seam exists and is tested.
- At least one concrete publish or authoring behavior is driven by tenant promotion-rule data.
- No dead placeholder promotion code remains.
