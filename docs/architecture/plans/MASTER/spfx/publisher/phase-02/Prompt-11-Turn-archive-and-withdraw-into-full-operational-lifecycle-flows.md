# Prompt-11 — Turn archive and withdraw into full operational lifecycle flows
...
Objective:
Replace simple state flips with complete archive/withdraw orchestration that accounts for page, binding, sync, history, and promotion side effects.
...
- binding/master/error/history seams
- tests

Required changes:
1. Define explicit archive and withdraw flows rather than treating them as bare state transitions.
2. Determine and implement the correct side effects for page visibility/state, binding status/sync fields, history writes, and promotion/rollup exposure.
3. Use tenant field names and tenant list seams only.
4. Add focused tests for archive and withdraw behavior.
...
- Bindings, history, and master article fields stay synchronized during those flows.
