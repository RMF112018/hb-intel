    # Prompt 01 — Replace manual project fields with an authoritative project picker

    ## Objective

    Execute **Wave 1 — Remove the highest-friction authoring burdens / Step 01** for the Article Publisher remediation program.

    Step goal:
    Replace the manual `Project ID` and `Project name` inputs with a searchable project selector backed by the HBCentral `Projects` list, then hydrate authoritative project context automatically.
...
- packages/ui-kit/src/HbcPeoplePicker/
    ...
    - Searchable project lookup component or adapter suitable for SPFx hosted use.
- Automatic hydration of `ProjectId`, `ProjectName`, and any approved project context fields after selection.
- Removal of normal author dependence on manual project identity entry.

    ## Acceptance criteria

    - Authors can create a project article without typing project identity manually.
- Validation rules still pass using authoritative project-backed values.
- The flow remains author-safe for partial configuration states.
