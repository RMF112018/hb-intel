    # Prompt 02 — Realign styling architecture with UI doctrine

    ## Objective

    Execute **Wave 3 — Product hardening, doctrine alignment, and maintainability / Step 02** for the Article Publisher remediation program.

    Step goal:
    Replace the current hardcoded styling posture with a doctrine-compliant, token-disciplined, visibly non-generic premium SPFx surface treatment.
...
- docs/reference/ui-kit/doctrine/UI-Doctrine-SPFx-Homepage-Overlay.md
    ...
    - Removal or sharp reduction of raw hex/rgb styling sprawl.
- Stronger token discipline and component/surface variant strategy.
- Premium styling that no longer reads as safe enterprise panel chrome.

    ## Acceptance criteria

    - The surface is clearly more productized and doctrine-aligned.
- Styling no longer depends on broad raw-value sprawl as the dominant approach.
- Visual outcomes are visibly non-generic and structurally stronger than the current admin-form posture.
