    # Prompt 01 — Replace body textarea with a real rich-text editor

    ## Objective

    Execute **Wave 2 — Upgrade the core editorial experience / Step 01** for the Article Publisher remediation program.

    Step goal:
    Replace the current `Body (HTML accepted)` textarea with a governed rich-text authoring experience that supports practical editorial formatting without exposing authors to raw HTML-oriented workflows.
...
- apps/hb-webparts/src/homepage/data/publisherAdapter/validation/validationEngine.ts
    ...
    - Rich-text authoring surface with formatting toolbar.
- Predictable output that remains safe for publish composition and SharePoint-hosted rendering.
- Improved writing ergonomics without degrading authoring safety.

    ## Acceptance criteria

    - Article body authoring no longer depends on a plain textarea.
- Authors can compose formatted body content without technical markup knowledge.
- Validation, preview, and publish still consume the authored content safely.
