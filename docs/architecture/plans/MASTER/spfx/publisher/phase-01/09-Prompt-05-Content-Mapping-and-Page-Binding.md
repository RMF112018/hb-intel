# 09 — Prompt 05 — Content Mapping and Page Binding

```text
You are working in the live local HB Intel repo.

Objective:
Implement the structured content-to-shell mapping layer and durable page-binding behavior for publish, republish, and approved regeneration scenarios.
...
- Map structured post data into the current v1 shell blocks.
- Banner/title mapping must respect the OOB Page Title/banner behavior present in the shell.
- Map subheading and body into the correct text blocks.
- Map team-member child rows into Team Viewer inputs.
- Map media child rows into the OOB image-gallery inputs.
- Implement binding creation/update logic that preserves page identity across ordinary republish.
- Track shell key/version and template key/version in the binding record.
- Explicitly define what triggers an in-place update versus a full regeneration.
- Do not silently create duplicate live pages for the same post.

Required outputs before you stop:
- content-mapping layer
- binding writer/update logic
- republish behavior
- regeneration decision rules
- evidence that mappings populate the correct shell-owned regions
```
