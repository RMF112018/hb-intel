...
- Supporting shared packages already exist at verified maturity levels:
  ...
  - `@hbc/features-project-hub` (v0.1.90, **active expansion** — SF21 health-pulse foundation plus contract-level type/logic implementations for Financial, Schedule, Constraints, Permits, Safety, Project Closeout, Project Startup, Subcontract Execution Readiness, and Project Warranty are present; Financial module has 29 UI components and 12+ business logic subdomains rendering at `/project-hub/:projectId/financial` but all data access is mock — no repository registration, no real data fetching, no data persistence; P3-E15 Quality Control planning family authored and reconciled as implementation-ready architecture)
...
Project switching uses a **smart same-section model**:
...
- Shared package readiness for:
  ...
  - `@hbc/features-project-hub` (active expansion — substantial Phase 3 contract-level and UI scaffold implementation landed; Financial has 29 UI components rendering with mock-only data access; Startup and later modules still require additional delivery)
...
2. **`@hbc/features-project-hub` maturity reconciliation**
   ...
   **Financial-specific maturity note (2026-03-28):** "Contract-level implementations" for Financial means: complete type system, 29 UI components (FinancialControlCenter orchestrator with ForecastSummaryPage, BudgetPage, CashFlowPage, BuyoutPage sub-surfaces plus headers, grids, panels, and rails), 12+ business logic subdomains (governance, versioning, budget-import, cash-flow, buyout, business-rules, integrations, cost-code-reference, annotations, spine-events, validation, computors), and 1,979 lines of unit tests across 19 test files. The PWA route at `/project-hub/:projectId/financial` renders `FinancialControlCenter` with project context. It does NOT mean: registered `IFinancialRepository`, real data fetching, data persistence, URL-routed sub-tools (sub-surface navigation uses `useState`-based `surfaceMode`), or data-connected spine adapters. Per [FIN-PR1](phase-3-deliverables/financial/FIN-PR1-Financial-Production-Readiness-Maturity-Model.md): module-level classification is **Stage 2 — Architecturally Defined** (constrained by Forecast Summary and GC/GR pending T04; majority of tools at Stage 3). See FIN-PR1 §3.2 for per-tool classifications and §4 for anti-overclaiming guidance.
