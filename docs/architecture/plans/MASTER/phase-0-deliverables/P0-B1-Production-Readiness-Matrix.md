...
| `@hbc/post-bid-autopsy` | Shared-Feature Primitive (C) | v0.0.1 scaffold-only | Blocks BD and Estimating adapter completion; blocks score-benchmark real usage | SF22 T08-T09 pending: BD adapter wizard/summary/list/dashboard UI surfaces and Estimating surfaces not implemented; lifecycle/storage orchestration incomplete |
| `@hbc/ai-assist` | Shared-Feature Primitive (C) | v0.0.1 scaffold-only | Blocks production AI-assisted workflows | Mandatory Pre-Implementation Research Directive not yet complete; Azure AI Foundry endpoint integration and Smart Insert/Trust Meter UI not implemented |
...
**Discrepancy resolved:** The task brief mentioned 50 workspace members, but the authoritative `current-state-map.md` (section 6, Summary Metrics) lists 56 total workspace members. Research confirmed the correct count:
