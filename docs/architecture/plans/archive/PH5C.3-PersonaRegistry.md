...
- [x] 5.C.3.1 `personaRegistry.ts` created with all 11 personas (6 base + 5 supplemental)
- [x] 5.C.3.2 All personas include: id, name, email, roles, permissions, description, category, tags, usageExample
- [x] 5.C.3.3 Base personas represent core business roles (Admin, Accounting, Estimating, Project, Executive, Member)
- [x] 5.C.3.4 Supplemental personas cover edge cases (PendingOverride, ExpiredSession, MultiRole, ReadOnly, DegradedMode)
- [x] 5.C.3.5 PERSONA_REGISTRY exports methods: getById, base, supplemental, all, default, byTag, byCategory
- [x] 5.C.3.6 All permissions align with registered features and authentication guard logic
- [x] 5.C.3.7 Reference documentation `docs/reference/auth/personas.md` created with persona table and examples
- [x] 5.C.3.8 Unit tests cover all query methods; ≥95% code coverage
- [x] 5.C.3.9 No duplicate persona IDs; all personas have valid permission sets
- [x] 5.C.3.10 `pnpm turbo run build --filter=@hbc/auth` succeeds; PersonaRegistry exports available

---

## Phase 5.C.3 Progress Notes

- 5.C.3.1 [COMPLETED] — PersonaRegistry creation with all personas
- 5.C.3.2 [COMPLETED] — Query method implementation
- 5.C.3.3 [COMPLETED] — Permission mapping
- 5.C.3.4 [COMPLETED] — Reference documentation

### Verification Evidence

- `pnpm turbo run build --filter=@hbc/auth` - [PASS, 2026-03-07]
- `pnpm turbo run test --filter=@hbc/auth` - [PASS, 2026-03-07]
- Coverage report ≥95% for personaRegistry - [PASS, 2026-03-07, `personaRegistry.ts` statements 100.00%, branches 100.00%, functions 100.00%]
- `docs/reference/auth/personas.md` exists and is complete - [PASS, 2026-03-07]
...
D-PH5C-04 traceability closed: applied production persona registry and reference blocks, created `packages/auth/src/mock/personaRegistry.ts` (11 personas + helper methods) and `docs/reference/auth/personas.md`, and wired `PERSONA_REGISTRY` through `@hbc/auth/dev` via `packages/auth/src/dev.ts`.
D-PH5C-05 verification evidence: `pnpm turbo run build --filter=@hbc/auth` PASS; `pnpm turbo run test --filter=@hbc/auth` PASS; `pnpm --filter @hbc/auth run test:coverage` PASS with `personaRegistry.ts` coverage 100% statements/branches/functions; registry query/integrity tests added in `packages/auth/src/mock/personaRegistry.test.ts`.
