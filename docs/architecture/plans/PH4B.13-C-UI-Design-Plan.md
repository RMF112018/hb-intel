**HB-Intel — Phase 4B: Post-Audit Remediation & Production Readiness Plan Completion Task 4B.13**  
...
## 4. Phase 4b.13 — Menu & Overlay Theme Adaptation (NEW P1 – Critical Contrast Fix)

**Effort:** 3 hours  
**Owner:** UI Architect  
**Acceptance Criteria:**

1. Project Picker, User Menu, Notification Bell, Command Palette, and all other popovers/menus render with correct dark/Field Mode background, text, and border colors.  
2. No hard-coded white backgrounds or light-mode styles remain in any overlay component.  
3. Theme toggle in User Menu instantly updates all open overlays without visual flash.  
4. Storybook examples for `HbcAppShell`, `HbcProjectSelector`, `HbcUserMenu`, and `HbcCommandPalette` demonstrate dark + Field Mode.  
5. `pnpm turbo run dev` in `apps/pwa` shows perfect contrast (verified in both Chrome and Edge).

**Implementation Steps:**

- Audit every component inside `packages/ui-kit/src/HbcAppShell/` that uses Fluent `Menu`, `Popover`, or `Dropdown`.  
- Ensure all are wrapped by the same `FluentProvider` instance (already present in `HbcAppShell.tsx`).  
- Add explicit `useHbcTheme()` hook where needed to force `theme` prop on overlay roots.  
- Remove any residual `backgroundColor: "white"` or similar overrides.  
- Update `HbcAppShell.stories.tsx` with dark/Field Mode variants.  
