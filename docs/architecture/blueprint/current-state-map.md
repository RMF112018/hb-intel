...
| SF26 shared-feature plans (10 files: `SF26-Saved-Views.md` through `SF26-T09-Testing-and-Deployment.md`) | **Historical Foundational** | Phase 3 Stage 5.4; T01–T09 complete 2026-03-23; package `@hbc/saved-views` v0.1.0 feature-complete |
...
| SF27 shared-feature plans (10 files: `SF27-Bulk-Actions.md` through `SF27-T09-Testing-and-Deployment.md`) | **Historical Foundational** | Phase 3 Stage 5.6; T01–T09 complete 2026-03-23; package `@hbc/bulk-actions` v0.1.0 feature-complete |
| `docs/architecture/adr/ADR-0123-bulk-actions.md` | **Permanent Decision Rationale** | SF27 bulk actions shared primitive architecture; 10 locked decisions (L-01–L-10) |
| `docs/how-to/developer/bulk-actions-adoption-guide.md` | **Living Reference (Diataxis)** | How-to quadrant; developer audience; bulk-actions module adoption |
| `docs/reference/bulk-actions/api.md` | **Living Reference (Diataxis)** | Reference quadrant; developer audience; bulk-actions API reference |
...
| T09 test suite (`backend/functions/src/`) | **Canonical Current-State** | G2 provisioning verification test suite (schema-contract, step3, step4, retry, idempotency tests + integration scaffolds); 35+ unit tests across 4 new files + env-gated integration `.todo()` registry in smoke.test.ts + 4 approval-to-provisioning integration tests covering saga reconciliation; produced by W0-G2-T09 and extended by Phase 3 deliverables |
...
| `docs/reference/spfx-surfaces/estimating-requester-surface.md` | **Living Reference (Diátaxis)** | Reference quadrant; SPFx surface audience; requester guided setup surface spec and route map; produced by W0-G4-T01; implemented with normalized API client, SignalR progress, and ReadyToProvision timing coverage |
...
| `apps/accounting/src/pages/ProjectReview*.tsx` | **Canonical Current-State** | Controller queue and structured review pages; `ProjectReviewQueuePage` (filtered queue table with tabs and project number column) and `ProjectReviewDetailPage` (structured review with approve/clarify/hold/route-to-admin actions, project-number capture on approval, post-approval lifecycle banners, structured ApiError display); produced by W0-G4-T03; code-complete and actionable against live backend |
| `apps/accounting/src/backend/AccountingBackendContext.tsx` | **Canonical Current-State** | Centralized backend provider — manages API client lifecycle, token factory resolution (SPFx audience-scoped → session-based → placeholder), Function App URL via runtimeConfig, production readiness assessment; pages consume via `useAccountingBackend()` hook; produced by Phase 10 (P10-05) |
| `apps/accounting/src/config/runtimeConfig.ts` | **Canonical Current-State** | Module-level runtime config singleton — `setRuntimeConfig()` stores shell-injected config, `getApiAudience()`/`getFunctionAppUrl()`/`getBackendMode()` resolve with 3-tier fallback (runtime → env → default/error), `checkProductionReadiness()` validates prerequisites; aligned with Project Setup pattern; produced by Phase 10 (P10-02) |
| `apps/accounting/config/package-solution.json` | **Canonical Current-State** | SPFx package manifest — declares `webApiPermissionRequests` (`resource: "hb-intel-api-production"`, `scope: "access_as_user"`), matching Project Setup; version `001.000.028`; canonical packaging path documented in [accounting-spfx-packaging-truth-freeze.md](../reviews/accounting-spfx-packaging-truth-freeze.md); produced by Phase 10 (P10-04), version bumped Phase 11 (P11-01) |
...
| `apps/pwa/src/routes/provisioning/ProvisioningProgressView.tsx` | **Canonical Current-State** | PWA provisioning progress view; step-by-step checklist with HbcStatusBadge per step, SignalR real-time events merged with API-polled status, terminal state banners, connection loss indicator; upgraded from scaffold to requester-facing surface; produced by W0-G5-T04 |
...
| `packages/shell/src/projectSwitching.ts` | **Canonical Current-State** | Phase 3 landing doctrine seam: route-aware project switching; same-section resolution with target-project Control Center fallback; saves departing-project return-memory without using target-project return-memory to override explicit root entry; implemented 2026-03-24 |
| `packages/shell/src/stores/projectHubPortfolioState.ts` | **Canonical Current-State** | Phase 3 landing doctrine seam: portfolio-root continuity store for Project Hub filters, search, sort, and scroll; separate from project-scoped return-memory; implemented 2026-03-24 |
...
| `apps/pwa/src/router/projectHubRouting.ts` and `apps/pwa/src/router/workspace-routes.ts` Project Hub routes | **Canonical Current-State** | Phase 3 landing doctrine route layer: `/project-hub` as permission-aware portfolio root, `/project-hub/{projectId}` as canonical project-scoped Control Center, same-project invalid-section fallback, projectId normalization, and in-shell no-access handling; implemented 2026-03-24 |
| `apps/pwa/src/pages/ProjectHubPage.tsx` | **Canonical Current-State** | Phase 3 Project Hub page surface: meaningful unscoped portfolio root, project-scoped Control Center view, Back to Portfolio behavior, and in-shell `@hbc/smart-empty-state` rendering for zero-access / invalid-context cases; implemented 2026-03-24 |
...
| Phase 5 admin plans (5 canonical docs under `phase-05/`) | **Canonical Current-State** | Operator-console baseline, route taxonomy (8-lane model), page ownership map, rehoming decisions, exit reconciliation; 8-lane shell live with 3 active lanes (Runs, Config, Health), 4 scaffold lanes, operator landing page, 3 legacy redirects; completed 2026-04-03 |
| Phase 6 admin plans (13 canonical docs under `phase-06/`) | **Canonical Current-State** | Prerequisite audit, install/bootstrap architecture, step model, checkpoint policy, contract slice, preflight validator, install orchestrator (19 steps), checkpoint lifecycle, post-install verification, setup wizard UX, install run UX, operator runbook, final reconciliation; 4 backend services, 2 frontend pages; completed 2026-04-03 |
...
| `packages/provisioning` | @hbc/provisioning | Provisioning API client, Zustand store, SignalR real-time, request lifecycle | Depends on auth, models | v0.3.2 | `packages/provisioning/README.md` |
...
| `packages/bulk-actions` | @hbc/bulk-actions | Shared bulk-actions — selection semantics, eligibility, chunked execution, mixed-result reporting | Depends on models, ui-kit; peer: react, react-dom, @tanstack/react-query | v0.1.0 | `packages/bulk-actions/README.md` |
...
| `packages/features/project-hub` | @hbc/features-project-hub | Project Hub — health pulse, Phase 3 module contracts, and baseline reports catalog/source-readiness seam | v0.1.90 |
...
| `apps/project-hub` | @hbc/spfx-project-hub | 4003 | @hbc/features-project-hub | v0.0.10 — live routed SPFx lane has browser-verified no-horizontal-overflow behavior on dashboard/module/escalation/fallback surfaces; deeper module table surfaces remain planned, not live |
...
Repo-truth note: `apps/project-hub` now resolves the SharePoint `siteUrl` through the canonical project registry during SPFx web part initialization before rendering project content. It also exposes the governed Stage 10.2 module lane (`/financial`, `/schedule`, `/constraints`, `/permits`, `/safety`, `/reports`, `/qc`, `/closeout`, `/startup`, `/subcontract-readiness`, `/warranty`) and the Stage 10.3 SPFx-to-PWA escalation map: explicit dashboard / home / canvas launches for portfolio root, My Work, work queue, activity, and advanced canvas administration; module launches for schedule and reports depth boundaries; and reports / executive-review launches for advanced recovery, comparison, history, and artifact-specific thread management. Stage 10.4 now wires the SPFx home canvas through a governed persistence adapter: immediate localStorage continuity plus SharePoint-list-backed online rehydrate and mirror sync on the project site. Stage 11.1 now audits the live routed page surfaces so the home route, governed module routes, and unresolved-context fallbacks all remain inside `WorkspacePageShell` with explicit UI-Kit-governed layout selection instead of relying on a blanket module detail wrapper. Stage 11.2 now classifies the live routed SPFx data surfaces explicitly: the home-route summary band uses the governed summary-strip / KPI pattern, and the current governed module-lane summary/action surfaces remain intentional card/list-view companion surfaces. Stage 11.3 now removes the remaining inline-style and hardcoded spacing/layout debt from the live routed SPFx surfaces and shared Project Hub lane surface, and backs that cleanup with lint verification for hex/rgb/pixel style literals plus direct-`@hbc/ui-kit` import discipline on the live SPFx lane scope. Stage 11.4 now exposes governed shell-level density control on the live routed home, governed module, and unresolved-context fallback surfaces, and the routed app plus shared SPFx lane surface consume the canonical `useDensity()` / `HBC_DENSITY_TOKENS` model for density-aware spacing and touch-target separation. Denser Financial, Schedule, Constraints, and Permits table-style zones remain planned selections rather than live routed SPFx runtime. When no registry-backed project context resolves, or when a review-artifact-specific launch has no artifact context, the surface fails safely in-shell instead of fabricating project identity, personalization state, or deep-link state.
