...
- Shared contract vocabulary: RACI tuple (`R`/`A`/`C`/`I`), `RESPONSIBILITY_CONTRACT_PARTIES` (Owner / Architect / GC / Subcontractor / Vendor / Other), source-mark normalization (only R/A/C/I map directly to final RACI; other source marks are not blindly converted), classification, criticality, domain, lifecycle, workflow-step type/state, exception code, health band, audit event type, evidence-link state, and eight-lane UI identifiers.
- Canonical fixture posture preserved end-to-end (`109` matrix items / `82` register entries / `27` exceptions / `98` source rows / `47` snapshot rows / `0` owner-contract active default obligations) across `@hbc/models`, the backend mock provider, and the SPFx fixture client.
