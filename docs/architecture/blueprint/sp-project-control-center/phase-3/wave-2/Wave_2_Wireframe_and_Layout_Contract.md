...
| `team-and-access`         | Three-lane lifecycle preview: Team Viewer Lane, Permission Request Lane, and Access Manager Lane, including current role/permission visibility, request/assignment preview, and approval/execution-status preview. | No permission mutation, no group mutation, no Entra/Teams mutation, no Graph/PnP/backend calls, no request persistence, no approval execution; all actions preview-only/disabled/fixture-driven (W2-ODR-016).                                                                         |
| `documents`               | Two-lane Document Control preview: Microsoft Files Lane (SharePoint Drive / SharePoint document libraries + OneDrive) plus External Document Systems Lane (Procore Files, Document Crunch, Adobe Sign).            | Microsoft lane actions are disabled/preview-only in Wave 2; external lane is launch/deep-link/missing-config/access-issue only; no live Graph/PnP/API, no upload/download/copy-link execution, no approval execution, no permission mutation, no sync/mirror/write-back (W2-ODR-013). |
