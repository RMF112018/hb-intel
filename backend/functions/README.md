...
- **Post-deploy parity proof (hard gate).** CI runs `scripts/verify-functions-live-parity.ts` after deploy. The verifier resolves the live host from Azure metadata and then requires all of the following to pass: expected identity from the manifest, `/api/health` public contract (`status`, `artifact`, `timestamp`) with no readiness-only fields, hosted Safety route truth (`ingest`, `ingest/preview`, `replay` must be non-404), `/api/health/ready` protected-route truth (no-auth status must be `200|401|403`, malformed bearer must be `401`), and deploy-stamped app settings (`HBC_FUNCTIONS_BUILD_VERSION`, `HBC_FUNCTIONS_BUILD_SHA`, `HBC_FUNCTIONS_BUILD_TIMESTAMP`). Optional admin-token probing (`--admin-token`) enforces `200` plus readiness contract keys.
