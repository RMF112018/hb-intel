...
app.http('adminLegacyFallbackReviewList', {
  methods: ['GET'],
  authLevel: 'anonymous',
  route: 'admin-api/legacy-fallback/review/records',
  handler: withAuth(
    withTelemetry(
      async (
        request: HttpRequest,
        ...
        try {
          ...
          const result = await service.listRecords({
            status: (request.query.get('status') ?? undefined) as
              ...
              | 'disabled'
              | undefined,
            confidence: (request.query.get('confidence') ?? undefined) as
              | 'high'
              | 'medium'
              | 'low'
              | 'none'
...
app.http('adminLegacyFallbackReviewGetRecord', {
  methods: ['GET'],
  authLevel: 'anonymous',
  route: 'admin-api/legacy-fallback/review/records/{recordId}',
  handler: withAuth(
    withTelemetry(
      async (
        request: HttpRequest,
...
app.http('adminLegacyFallbackReviewManualBind', {
  methods: ['POST'],
  authLevel: 'anonymous',
  route: 'admin-api/legacy-fallback/review/records/{recordId}/bind',
  handler: withAuth(
    withTelemetry(
      async (
        request: HttpRequest,
        ...
        const matchedProjectTitle =
          typeof body.matchedProjectTitle === 'string' ? body.matchedProjectTitle.trim() : '';
        if (!Number.isInteger(matchedProjectListItemId) || matchedProjectListItemId <= 0) {
          return errorResponse(
            400,
            'VALIDATION_ERROR',
            'matchedProjectListItemId must be a positive integer',
            requestId,
          );
        }
        if (!matchedProjectTitle) {
          return errorResponse(
            400,
            'VALIDATION_ERROR',
            'matchedProjectTitle is required',
            requestId,
          );
        }
...
app.http('adminLegacyFallbackReviewIgnore', {
  methods: ['POST'],
  authLevel: 'anonymous',
  route: 'admin-api/legacy-fallback/review/records/{recordId}/ignore',
  handler: withAuth(
    withTelemetry(
      async (
        request: HttpRequest,
...
app.http('adminLegacyFallbackReviewDisable', {
  methods: ['POST'],
  authLevel: 'anonymous',
  route: 'admin-api/legacy-fallback/review/records/{recordId}/disable',
  handler: withAuth(
    withTelemetry(
      async (
        request: HttpRequest,
...
app.http('adminLegacyFallbackReviewRevalidate', {
  methods: ['POST'],
  authLevel: 'anonymous',
  route: 'admin-api/legacy-fallback/review/revalidate',
  handler: withAuth(
    withTelemetry(
