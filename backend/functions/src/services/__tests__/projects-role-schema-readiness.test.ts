...
describe('field list constants', () => {
  it('Projects required fields match the canonical 14 from @hbc/models', () => {
    expect(MY_PROJECT_LINKS_REQUIRED_FIELDS_PROJECTS).toEqual(
      MY_PROJECT_ASSIGNMENT_INTERNAL_FIELDS,
    );
    expect(MY_PROJECT_LINKS_REQUIRED_FIELDS_PROJECTS).toHaveLength(14);
  });

  it('Legacy Registry required fields are the canonical 14 plus procoreProject', () => {
    expect(MY_PROJECT_LINKS_REQUIRED_FIELDS_LEGACY_REGISTRY).toHaveLength(15);
...
describe('buildProjectsRoleSchemaReadinessReport — missing fields', () => {
  ...
  it('reports every required field as missing on an empty Registry snapshot', () => {
    ...
    expect(report.ready).toBe(false);
    expect(report.legacyRegistry.entries).toHaveLength(15);
    for (const entry of report.legacyRegistry.entries) {
...
describe('buildProjectsRoleSchemaReadinessReport — list metadata', () => {
  ...
  it('contains exactly 14 entries for Projects and 15 for Legacy Registry regardless of input snapshot size', () => {
    const padded = [
    ...
    expect(report.projects.entries).toHaveLength(14);
    expect(report.legacyRegistry.entries).toHaveLength(15);
  });
});
