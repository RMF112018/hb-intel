/**
 * Provisioner for the four `MyDashboardAdobeSign*` SharePoint cache lists on
 * the MyDashboard site, backing the B05.15 Adobe Sign webhook-backed
 * projection cache program.
 *
 * Default behavior is dry-run (plan + report only). `--apply` runs the
 * create-list-if-missing path, reconciles columns against each list's
 * descriptor in `MY_DASHBOARD_ADOBE_SIGN_CACHE_LIST_DESCRIPTORS`, and sets
 * `EnforceUniqueValues=true` on the indexed primary-key column of each list
 * (`AdobeActorKey` on UserCache, `ProjectionKey` on AgreementProjectionCache,
 * `SubscriptionKey` on WebhookSubscriptions, `RunId` on SyncRuns). Wrong-type
 * and wrong-unique drift refuse `--apply` unless `--allow-type-drift` is
 * supplied.
 *
 * The provisioner does NOT script the list permission-inheritance break.
 * Permissions are an operator step documented in
 * `docs/how-to/administrator/provision-my-dashboard-adobe-sign-cache-lists.md`.
 *
 * Operator usage:
 *
 *   pnpm tsx scripts/provision-my-dashboard-adobe-sign-cache-lists.ts                  # dry-run (text)
 *   pnpm tsx scripts/provision-my-dashboard-adobe-sign-cache-lists.ts --json           # dry-run (JSON)
 *   pnpm tsx scripts/provision-my-dashboard-adobe-sign-cache-lists.ts --apply --json   # create+reconcile
 *
...
export function parseArgs(argv: readonly string[]): IProvisionArgs {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    if (token === '--help' || token === '-h') {
      throw new Error(
        'Usage: provision-my-dashboard-adobe-sign-cache-lists [--apply] [--json] [--allow-type-drift] [--site-url <url>]',
      );
    }
    throw new Error(
...
export function resolveSiteUrl(args: IProvisionArgs, env: NodeJS.ProcessEnv): string {
  const resolved =
    args.siteUrl ??
    env.SHAREPOINT_MYDASHBOARD_SITE_URL ??
    getMyDashboardAdobeSignCacheListHostSiteUrl();
  if (!resolved) {
...
function nextCommandsFor(
  ...
  if (report.success && !report.apply) {
    return [
      `pnpm tsx scripts/provision-my-dashboard-adobe-sign-cache-lists.ts --apply ${siteArg} --json`,
      'pnpm tsx scripts/verify-my-dashboard-adobe-sign-cache-lists.ts --json',
      'Operator: break list permission inheritance and apply restricted role assignments on each of the four cache lists (backend-mediated; no end-user write access).',
    ];
  }
  if (!report.success && report.hasBlockingDrift) {
    return [
      ...
      `pnpm tsx scripts/provision-my-dashboard-adobe-sign-cache-lists.ts ${siteArg} --json`,
    ];
  }
  return [
    `pnpm tsx scripts/provision-my-dashboard-adobe-sign-cache-lists.ts ${siteArg} --json`,
...
export function buildProvisioningReport(input: {
  ...
  const base = {
    ...
    governanceReminder:
      'The four MyDashboardAdobeSign* cache lists MUST have permission inheritance broken with restricted role assignments before any production seed/cutover. This script does NOT script permission changes — operator must perform that step manually.',
...
export function formatReport(report: IProvisionReport, asJson: boolean): string {
  ...
  const lines: string[] = [];
  lines.push('[provision-my-dashboard-adobe-sign-cache-lists]');
...
export async function main(args: IProvisionArgs, deps: IMainDeps, siteUrl: string): Promise<0 | 1> {
  ...
  const targets: IProvisionTargetReport[] = [];
  for (const descriptor of MY_DASHBOARD_ADOBE_SIGN_CACHE_LIST_DESCRIPTORS) {
    const target = await reconcileTarget(descriptor, args, deps.listAdapter);
    targets.push(target);
  }
...
async function runFromCli(): Promise<void> {
  ...
  const { createPnPRegistryListAdapter, resolveSharePointBearerToken } = await import(
    '../backend/functions/src/services/my-projects-projection/registry-pnp-list-adapter.js'
  );
  ...
  const code = await withTimeout(
    ...
    180_000,
    'provisioning MyDashboard Adobe Sign cache lists',
  );
  process.exit(code);
}
...
const invokedDirectly =
  ...
  process.argv[1].endsWith('provision-my-dashboard-adobe-sign-cache-lists.ts');

if (invokedDirectly) {
  process.on('unhandledRejection', (err) => {
    console.error(
      `provision-my-dashboard-adobe-sign-cache-lists: unhandled rejection — ${err instanceof Error ? err.message : String(err)}`,
    );
    process.exit(1);
  });
  process.on('uncaughtException', (err) => {
    console.error(
      `provision-my-dashboard-adobe-sign-cache-lists: uncaught exception — ${err instanceof Error ? err.message : String(err)}`,
    );
    process.exit(1);
  });
