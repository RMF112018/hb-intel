...
export interface IMainDeps {
  readonly listAdapter: IStorageListAdapter;
  readonly now: () => string;
...
export function parseArgs(argv: readonly string[]): IProvisionArgs {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    if (token === '--help' || token === '-h') {
      throw new Error(
        'Usage: provision-my-projects-projection-storage [--apply] [--json] [--allow-type-drift] [--site-url <url>]',
      );
    }
    throw new Error(
...
function nextCommandsFor(
  ...
  if (report.success && !report.apply) {
    return [
      `pnpm tsx scripts/provision-my-projects-projection-storage.ts --apply ${siteArg} --json`,
      'pnpm tsx scripts/verify-my-projects-projection-storage.ts --json',
      'Operator: break list permission inheritance and apply restricted role assignments to all seven projection lists before production use.',
    ];
  }
  if (!report.success && report.hasBlockingDrift) {
    return [
      ...
      `pnpm tsx scripts/provision-my-projects-projection-storage.ts ${siteArg} --json`,
    ];
  }
  return [
    `pnpm tsx scripts/provision-my-projects-projection-storage.ts ${siteArg} --json`,
...
export function buildProvisioningReport(input: {
  ...
  const base = {
    ...
    governanceReminder:
      'All seven projection/control/diagnostic lists MUST have permission inheritance broken with restricted role assignments before production use. This script does NOT apply permission changes.',
...
export function formatReport(report: IProvisionReport, asJson: boolean): string {
  ...
  const lines: string[] = [];
  lines.push('[provision-my-projects-projection-storage]');
...
async function reconcileTarget(
  ...
  args: IProvisionArgs,
  listAdapter: IStorageListAdapter,
  ...
  if (args.apply && !driftBlocks) {
    for (const plan of plans) {
      ...
      } else if (plan.kind === 'update-settings') {
...
export async function main(args: IProvisionArgs, deps: IMainDeps, siteUrl: string): Promise<0 | 1> {
  ...
  for (const descriptor of MY_PROJECTS_PROJECTION_STORAGE_DESCRIPTOR) {
    targets.push(await reconcileTarget(descriptor, args, deps.listAdapter));
  }
...
export async function runProvisionCli(
  ...
  const { createPnPStorageListAdapter, resolveSharePointBearerToken } = await import(
    '../backend/functions/src/services/my-projects-projection/storage-pnp-list-adapter.js'
  );
  const adapter = createPnPStorageListAdapter({
    hostSiteUrl: siteUrl,
